...
public class EventLoader {
    ...
    private static class LoadEventDaysRequest implements LoadRequest {
        ...
        public void processRequest(EventLoader eventLoader) {
            ...
            Cursor cursor = null;
            if (Utils.checkSelfPermission(eventLoader.mContext,
                    Manifest.permission.READ_CALENDAR)) {
                cursor = EventDays.query(cr, startDay, numDays, PROJECTION);
                try {
                    int startDayColumnIndex = cursor
                            .getColumnIndexOrThrow(EventDays.STARTDAY);
                    int endDayColumnIndex = cursor
                            .getColumnIndexOrThrow(EventDays.ENDDAY);
    ...
    private static class LoadEventsRequest implements LoadRequest {
        ...
        public LoadEventsRequest(int id, int startDay, int numDays,
                ArrayList<Event> events, final Runnable successCallback,
        ...
        public void processRequest(EventLoader eventLoader) {
            Event.loadEvents(eventLoader.mContext, events, startDay, numDays,
                    id, eventLoader.mSequenceNumber);
     ...
     */
    public void loadEventsInBackground(final int numDays,
            final ArrayList<Event> events, int startDay,
        ...
        LoadEventsRequest request = new LoadEventsRequest(id, startDay,
    ...
    void loadEventDaysInBackground(int startDay, int numDays,
        ...
        LoadEventDaysRequest request = new LoadEventDaysRequest(startDay,
