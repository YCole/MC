...
public class AlertUtils {
    ...
    // other notifications (the 'active' future/concurrent ones) use a unique
    // ID.
    ...
    // A flag for using local storage to save alert state instead of the alerts
    // DB table.
    // This allows the unbundled app to run alongside other calendar apps
    // without eating
    // alerts from other apps.
    static boolean BYPASS_DB = true;

    // SharedPrefs table name for storing fired alerts. This prevents other
    // installed
    // Calendar apps from eating the alerts.
    private static final String ALERTS_SHARED_PREFS_NAME = "calendar_alerts";

    // Keyname prefix for the alerts data in SharedPrefs. The key will contain a
    // combo
    // of event ID, begin time, and alarm time. The value will be the fired
    // time.
    ...
    // The # of days to save alert states in the shared prefs table, before
    // flushing. This
    // can be any value, since AlertService will also check for a recent
    // alertTime before
    // ringing the alert.
    private static final int FLUSH_INTERVAL_DAYS = 1;
    private static final int FLUSH_INTERVAL_MS = FLUSH_INTERVAL_DAYS * 24 * 60
            * 60 * 1000;

    /**
     * Creates an AlarmManagerInterface that wraps a real AlarmManager. The
     * alarm code was abstracted to an interface to make it testable.
     */
    public static AlarmManagerInterface createAlarmManager(Context context) {
        ...
        return new AlarmManagerInterface() {
            @Override
            public void set(int type, long triggerAtMillis,
    ...
    public static void scheduleAlarm(Context context,
            AlarmManagerInterface manager, long alarmTime) {
        scheduleAlarmHelper(context, manager, alarmTime, false);
    }

    /**
     * Schedules the next alarm to silently refresh the notifications. Note that
     * if there is a pending silent refresh alarm, it will be replaced with this
     * one.
     */
    static void scheduleNextNotificationRefresh(Context context,
            AlarmManagerInterface manager, long alarmTime) {
        scheduleAlarmHelper(context, manager, alarmTime, true);
    }

    private static void scheduleAlarmHelper(Context context,
            AlarmManagerInterface manager, long alarmTime, boolean quietUpdate) {
        ...
        if (quietUpdate) {
            ...
            // Set data field so we get a unique PendingIntent instance per
            // alarm or else alarms
            // may be dropped.
    ...
    /**
     * Format the second line which shows time and location for single alert or
     * the number of events for multiple alerts 1) Show time only for non-all
     * day events 2) No date for today 3) Show "tomorrow" for tomorrow 4) Show
     * date for days beyond that
     */
    static String formatTimeLocation(Context context, long startMillis,
        ...
        int eventDay = Time.getJulianDay(time.toMillis(false), allDay ? 0
                : time.gmtoff);
        ...
        StringBuilder sb = new StringBuilder(Utils.formatDateRange(context,
    ...
    public static ContentValues makeContentValues(long eventId, long begin,
    ...
    public static Intent buildEventViewIntent(Context c, long eventId,
            long begin, long end) {
    ...
    public static SharedPreferences getFiredAlertsTable(Context context) {
        return context.getSharedPreferences(ALERTS_SHARED_PREFS_NAME,
    ...
    /**
     * Returns whether the SharedPrefs storage indicates we have fired the alert
     * before.
     */
    static boolean hasAlertFiredInSharedPrefs(Context context, long eventId,
     ...
     */
    static void setAlertFiredInSharedPrefs(Context context, long eventId,
            long beginTime, long alarmTime) {
        // Store alarm time as the value too so we don't have to parse all the
        // keys to flush
        // old alarms out of the table later.
        SharedPreferences prefs = getFiredAlertsTable(context);
        SharedPreferences.Editor editor = prefs.edit();
        editor.putLong(getFiredAlertsKey(eventId, beginTime, alarmTime),
                alarmTime);
        editor.apply();
    }

    /**
     * Scans and flushes the internal storage of old alerts. Looks up the
     * previous flush time in SharedPrefs, and performs the flush if overdue.
     * Otherwise, no-op.
     */
                ...
                for (Map.Entry<String, ?> entry : prefs.getAll().entrySet()) {
                    ...
                    if (key.startsWith(KEY_FIRED_ALERT_PREFIX)) {
                        ...
                        if (value instanceof Long) {
                            ...
                            // Should never occur.
                            Log.e(TAG, "SharedPrefs key " + key
                                    + " did not have Long value: " + value);
                            continue;
                        }
    ...
    private static int getIntervalInDays(long startMillis, long endMillis,
            Time timeObj) {
