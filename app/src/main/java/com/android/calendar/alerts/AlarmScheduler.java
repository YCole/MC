...
/**
 * Schedules the next EVENT_REMINDER_APP broadcast with AlarmManager, by
 * querying the events and reminders tables for the next upcoming alert.
 */
public class AlarmScheduler {
    ...
    static final String[] INSTANCES_PROJECTION = new String[] {
            Instances.EVENT_ID, Instances.BEGIN, Instances.ALL_DAY, };
    ...
    // Add a slight delay for the EVENT_REMINDER_APP broadcast for a couple
    // reasons:
    // (1) so that the concurrent reminder broadcast from the provider doesn't
    // result
    // in a double ring, and (2) some OEMs modified the provider to not add an
    // alert to
    // the CalendarAlerts table until the alert time, so for the unbundled app's
    // notifications to work on these devices, a delay ensures that AlertService
    // won't
    // read from the CalendarAlerts table until the alert is present.
    static final int ALARM_DELAY_MS = 1000;

    // The reminders query looks like
    // "SELECT ... AND eventId IN 101,102,202,...". This
    // sets the max # of events in the query before batching into multiple
    // queries, to
    // limit the SQL query length.
    private static final int REMINDER_QUERY_BATCH_SIZE = 50;

    // We really need to query for reminder times that fall in some interval,
    // but
    // the Reminders table only stores the reminder interval (10min, 15min,
    // etc), and
    // we cannot do the join with the Events table to calculate the actual alert
    // time
    // from outside of the provider. So the best we can do for now consider
    // events
    // whose start times begin within some interval (ie. 1 week out). This means
    // reminders which are configured for more than 1 week out won't fire on
    // time. We
    // can minimize this to being only 1 day late by putting a 1 day max on the
    // alarm time.
     ...
     * 
     * This is historically done in the provider but we dupe this here so the
     * unbundled app will work on devices that have modified this portion of the
     * provider. This has the limitation of querying events within some interval
     * from now (ie. looks at reminders for all events occurring in the next
     * week). This means for example, a 2 week notification will not fire on
     * time.
     */
    ...
    static void scheduleNextAlarm(Context context,
            AlarmManagerInterface alarmManager, int batchSize,
            long currentMillis) {
        Cursor instancesCursor = null;
        try {
            instancesCursor = queryUpcomingEvents(context,
                    context.getContentResolver(), currentMillis);
            if (instancesCursor != null) {
                queryNextReminderAndSchedule(instancesCursor, context,
                        context.getContentResolver(), alarmManager, batchSize,
                        currentMillis);
            }
        } finally {
            if (instancesCursor != null) {
     ...
     */
    private static Cursor queryUpcomingEvents(Context context,
            ContentResolver contentResolver, long currentMillis) {
        Time time = new Time();
        ...
        ContentUris.appendId(uriBuilder, localStartMin
                - DateUtils.DAY_IN_MILLIS);
        ContentUris.appendId(uriBuilder, localStartMax
                + DateUtils.DAY_IN_MILLIS);
        ...
        if (Utils.checkSelfPermission(context,
            ...
            cursor = contentResolver.query(uriBuilder.build(),
                    INSTANCES_PROJECTION, queryBuilder.toString(), queryArgs,
                    null);
        }
        return cursor;
    }
    ...
    /**
     * Queries for all the reminders of the events in the instancesCursor, and
     * schedules the alarm for the next upcoming reminder.
     */
    private static void queryNextReminderAndSchedule(Cursor instancesCursor,
        ...
        if (AlertService.DEBUG) {
            ...
            if (eventCount == 0) {
                ...
                Log.d(TAG,
                        "Query result count for events starting within 1 week: "
                                + eventCount);
            }
        }

        // Put query results of all events starting within some interval into
        // map of event ID to
        // local start time.
        ...
        while (!instancesCursor.isAfterLast()) {
            ...
            while (index++ < batchSize && instancesCursor.moveToNext()) {
                ...
                boolean allday = instancesCursor
                        .getInt(INSTANCES_INDEX_ALL_DAY) != 0;
                long localStartTime;
                if (allday) {
                    // Adjust allday to local time.
                    localStartTime = Utils.convertAlldayUtcToLocal(timeObj,
                            begin, Time.getCurrentTimezone());
                } else {
                ...
                if (Log.isLoggable(TAG, Log.DEBUG)) {
                    ...
                    msg.append("Events cursor result -- eventId:").append(
                            eventId);
                    msg.append(", allDay:").append(allday);
                    msg.append(", start:").append(localStartTime);
                    msg.append(" (")
                            .append(timeObj.format("%a, %b %d, %Y %I:%M%P"))
                            .append(")");
            ...
            if (Utils.checkSelfPermission(context,
                    Manifest.permission.READ_CALENDAR)) {
                try {
                    cursor = contentResolver.query(Reminders.CONTENT_URI,
                            REMINDERS_PROJECTION, REMINDERS_WHERE
                                    + eventIdsForQuery, null, null);
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        int reminderMinutes = cursor
                                .getInt(REMINDERS_INDEX_MINUTES);
                        List<Long> startTimes = eventMap.get(eventId);
                        if (startTimes != null) {
                            for (Long startTime : startTimes) {
                                long alarmTime = startTime - reminderMinutes
                                        * DateUtils.MINUTE_IN_MILLIS;
                                if (alarmTime > currentMillis
                                ...
                                if (Log.isLoggable(TAG, Log.DEBUG)) {
                                    ...
                                    StringBuilder msg = new StringBuilder();
                                    msg.append(
                                            "Reminders cursor result -- eventId:")
                                            .append(eventId);
                                    msg.append(", startTime:")
                                            .append(startTime);
                                    msg.append(", minutes:").append(
                                            reminderMinutes);
                                    msg.append(", alarmTime:")
                                            .append(alarmTime);
                                    msg.append(" (")
                                            .append(timeObj
        ...
        if (nextAlarmTime < Long.MAX_VALUE) {
            scheduleAlarm(context, nextAlarmEventId, nextAlarmTime,
                    currentMillis, alarmManager);
        }
    }

    /**
     * Schedules an alarm for the EVENT_REMINDER_APP broadcast, for the
     * specified alarm time with a slight delay (to account for the possible
     * duplicate broadcast from the provider).
     */
    private static void scheduleAlarm(Context context, long eventId,
            long alarmTime, long currentMillis,
            AlarmManagerInterface alarmManager) {
        // Max out the alarm time to 1 day out, so an alert for an event far in
        // the future
        // (not present in our event query results for a limited range) can only
        // be at
        // most 1 day late.
        ...
        if (AlertService.DEBUG) {
            ...
            Log.d(TAG,
                    "Scheduling alarm for EVENT_REMINDER_APP broadcast for event "
                            + eventId + " at " + alarmTime + " (" + schedTime
                            + ")");
        }

        // Schedule an EVENT_REMINDER_APP broadcast with AlarmManager. The extra
        // is
        // only used by AlertService for logging. It is ignored by
        // Intent.filterEquals,
        // so this scheduling will still overwrite the alarm that was previously
        // pending.
        // Note that the 'setClass' is required, because otherwise it seems the
        // broadcast
