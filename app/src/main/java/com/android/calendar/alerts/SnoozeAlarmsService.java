...
public class SnoozeAlarmsService extends IntentService {
    ...
    public void onHandleIntent(Intent intent) {
        ...
        // The ID reserved for the expired notification digest should never be
        // passed in
        ...
        if (eventId != -1) {
            ...
            dismissValues.put(PROJECTION[COLUMN_INDEX_STATE],
                    CalendarAlerts.STATE_DISMISSED);
            resolver.update(uri, dismissValues, selection, null);

            // Add a new alarm
            long alarmTime = System.currentTimeMillis()
                    + AlertUtils.SNOOZE_DELAY;
            ContentValues values = AlertUtils.makeContentValues(eventId,
                    eventStart, eventEnd, alarmTime, 0);
            resolver.insert(uri, values);
            AlertUtils.scheduleAlarm(SnoozeAlarmsService.this,
                    AlertUtils.createAlarmManager(this), alarmTime);
        }
