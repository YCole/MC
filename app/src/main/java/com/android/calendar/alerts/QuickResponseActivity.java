...
/**
 * Activity which displays when the user wants to email guests from
 * notifications.
 * 
 * This presents the user with list if quick responses to be populated in an
 * email to minimize typing.
 * 
 */
public class QuickResponseActivity extends ListActivity implements
        OnItemClickListener {
    ...
    protected void onCreate(Bundle icicle) {
        ...
        mResponses[i] = getResources().getString(
                R.string.quick_response_custom_msg);

        setListAdapter(new ArrayAdapter<String>(this,
                R.layout.quick_response_item, mResponses));
    }
    ...
    private class QueryThread extends Thread {
        ...
        @Override
        public void run() {
            Intent emailIntent = AlertReceiver.createEmailIntent(
                    QuickResponseActivity.this, mEventId, mBody);
            if (emailIntent != null) {
                try {
                    startActivity(emailIntent);
                    finish();
                } catch (ActivityNotFoundException ex) {
                    QuickResponseActivity.this.getListView().post(
                            new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(
                                            QuickResponseActivity.this,
                                            R.string.quick_response_email_failed,
                                            Toast.LENGTH_LONG);
                                    finish();
                                }
                            });
