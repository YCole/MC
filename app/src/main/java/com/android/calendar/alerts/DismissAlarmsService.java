...
public class DismissAlarmsService extends IntentService {
    ...
    public void onHandleIntent(Intent intent) {
        ...
        long[] eventStarts = intent
                .getLongArrayExtra(AlertUtils.EVENT_STARTS_KEY);
        int notificationId = intent.getIntExtra(AlertUtils.NOTIFICATION_ID_KEY,
                -1);
        if (AlertService.DEBUG) {
            Log.d(TAG, "lxg --onReceive: a=" + intent.getAction() + " "
                    + intent.toString() + ", id= " + eventId
        ...
        // Dismiss a specific fired alarm if id is present, otherwise, dismiss
        // all alarms
        if (eventId != -1) {
        ...
        values.put(PROJECTION[COLUMN_INDEX_STATE],
        ...
        if (SHOW_ACTION.equals(intent.getAction())) {
            // Show event on Calendar app by building an intent and task stack
            // to start
            // EventInfoActivity with AllInOneActivity as the parent activity
            // rooted to home.
            Intent i = AlertUtils.buildEventViewIntent(this, eventId,
                    eventStart, eventEnd);

            TaskStackBuilder.create(this)
                    .addParentStack(EventInfoActivity.class).addNextIntent(i)
                    .startActivities();
        }
    }
