...
public class CalendarController {
    ...
    public void setMonthView(ImageView monthNum) {
        mMonthView = monthNum;
    ...
    public interface ViewType {
        ...
        /* modify start liaozhenbin 2017/6/21 */
        final int YEAR = 6;
        /* modify end liaozhenbin 2017/6/21 */
    }
    ...
    public static class EventInfo {
        ...
        public boolean isAllDay() {
            if (eventType != EventType.VIEW_EVENT) {
                Log.wtf(TAG, "illegal call to isAllDay , wrong event type "
                        + eventType);
                return false;
            }
        ...
        public int getResponse() {
            if (eventType != EventType.VIEW_EVENT) {
                Log.wtf(TAG, "illegal call to getResponse , wrong event type "
                        + eventType);
    ...
    /* HCT_MODIFY lixiange jump date hilight begin */
    public static final long EXTRA_JUMP_DATE = 3;
    /* HCT_MODIFY lixiange jump date hilight end */
    public static final long EXTRA_GOTO_BACK_TO_PREVIOUS = 4;
    public static final long EXTRA_GOTO_TODAY = 8;

    public static final long EXTRA_GOTO_USER_TOUCH = 16;
    ...
    public void sendEvent(Object sender, long eventType, Time start, Time end,
        ...
        info.extraLong = extraLong;
        Log.d("liao", "selectedTime: " + selected);
        Log.d("liao", eventInfoToString(info));
        this.sendEvent(sender, info);
    }
    ...
    public void sendEvent(Object sender, final EventInfo event) {
        ...
        if (event.selectedTime != null
                && event.selectedTime.toMillis(false) != 0) {
            mTime.set(event.selectedTime);
        } else {
            if (startMillis != 0) {
                // selectedTime is not set so set mTime to startTime if it is
                // not
        ...
        if (!handled) {
            ...
            if (DEBUG) {
                Log.d(TAG, "sendEvent: handled = " + handled);
            }
        }
    }
    ...
    public void registerEventHandler(int key, EventHandler eventHandler) {
        synchronized (this) {
            if (mDispatchInProgressCounter > 0) {
                ...
                if (mToBeAddedEventHandlers.containsKey(key)) {
                    mToBeAddedEventHandlers.remove(key);
                }
    ...
    public void deregisterEventHandler(Integer key) {
        synchronized (this) {
            try {
                if (mDispatchInProgressCounter > 0) {
                    // To avoid ConcurrencyException, stash away the event
                    // handler for now.
    ...
    public void deregisterAllEventHandlers() {
        synchronized (this) {
            try {
                ...
                Log.e(TAG, "Error: deregisterAllEventHandlers: " + e);
            }
        }
    }
    ...
    private void launchSettings() {
        Intent intent = new Intent(Intent.ACTION_VIEW);
        // intent.setClass(mContext, GeneralPreferences.class);
        intent.setClass(mContext, CalendarSettings.class);
    ...
    private String eventInfoToString(EventInfo eventInfo) {
        String tmp = "Unknown";
        Log.d("liao", eventInfo.eventType + "");
        ...
        if ((eventInfo.eventType & EventType.GO_TO) != 0) {
            ...
            tmp = "Update title";
        } else if ((eventInfo.eventType & EventType.MONTH_GO_TO) != 0) {
            tmp = "GO to -->";
        }
        builder.append(tmp);
        builder.append(": id=");
