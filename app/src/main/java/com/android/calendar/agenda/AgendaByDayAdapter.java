...
public class AgendaByDayAdapter extends BaseAdapter {
    ...
    // HCT_MODIFY lixiange MF3.0 add delete all select event caculate the number
    // of the agendasize */
    ...
    /** multiagenda begin */
    public HashSet<Integer> getMultiAgendaPosations(int position) {
        int len = mRowInfo.size();
        HashSet<Integer> agendaposItems = new HashSet<Integer>();
        int agendapos = mRowInfo.get(position).mPosition;
        for (int index = 0; index < len; index++) {
            RowInfo row = mRowInfo.get(index);
            if (row.mPosition == agendapos && row.mType == TYPE_MEETING) {
                agendaposItems.add(index);
            }
        }
        return agendaposItems;
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (row.mType == TYPE_DAY) {
            ...
            mStringBuilder.setLength(0);
            /* HCT_MODIFY start longgang for 340IV 2 */
            // flags = DateUtils.FORMAT_SHOW_DATE;
            flags = DateUtils.FORMAT_SHOW_DATE | DateUtils.FORMAT_SHOW_YEAR;
            /* HCT_MODIFY end longgang for 340IV 2 */
             ...
             * agendaDayView.setBackgroundResource
             * (R.drawable.agenda_item_bg_primary); holder.grayed = false; }
             * else { agendaDayView.setBackgroundResource(R.drawable.
             * agenda_item_bg_secondary); holder.grayed = true; }
             */
            /*
             * View view1 = (View)agendaDayView.findViewById(R.id.top_divider);
             * View view2 =
             * (View)agendaDayView.findViewById(R.id.top_divider_simple);
             * view1.setVisibility(View.VISIBLE);
             * view2.setVisibility(View.VISIBLE); if (first){ first=false;
             * position_former = mRowInfo.size(); } //int position_former =
             * mRowInfo.size(); //int minJulianDay =
             * mAgendaListView.getFragment().getSlideRangeMinJulianDay(); int
             * diff = mRowInfo.size() - position_former; position_former+=diff;
             * Log.v("rjcposition","position=="+position);
             * Log.v("rjcposition","position_former=="+position_former);
             * Log.v("rjcposition","diff=="+diff);
             * 
             * if (position == 0){ int position_latter = position_former;
             * view1.setVisibility(View.GONE); view2.setVisibility(View.GONE);
             * }else if(position==diff && diff!=0){
             * view1.setVisibility(View.VISIBLE);
            ...
            View agendalist = (View) itemView
                    .findViewById(R.id.agenda_list_divider);

            if (position + 1 == mRowInfo.size()) {
                agendalist.setVisibility(View.GONE);
            } else if (position + 1 < mRowInfo.size()) {
                RowInfo rowInfonext = mRowInfo.get(position + 1);
                if (rowInfonext.mType == TYPE_DAY) {
                    agendalist.setVisibility(View.GONE);
                } else {
            ...
            // set the background
            CheckBoxHCT selectedMarker = (CheckBoxHCT) itemView
                    .findViewById(R.id.selected_marker);
            selectedMarker.SetColor(mContext.getResources().getColor(
                    R.color.cale_checkbox_color));
            selectedMarker.setChecked(holder.selected);
            if ((!allDay && row.mEventStartTimeMilli <= System
                    .currentTimeMillis())
                    || (allDay && row.mDay <= mTodayJulianDay)) {
                // itemView.setBackgroundResource(R.drawable.agenda_item_bg_primary);//agenda_item_bg_secondary);
                itemView.setBackgroundColor(0xfffafafa);
                title.setTypeface(Typeface.DEFAULT);
                holder.grayed = true;
                holder.title.setTextColor(0x50333333);
                holder.when.setTextColor(0x50999999);
                holder.where.setTextColor(0x50999999);
                if (holder.selected) {
                    /***
                     * HCT_MODIFY jianghejie to make the listitem 's press color
                     * can work
                     */
                    // View
                    // selectedMarker=itemView.findViewById(R.id.selected_marker);
                    // selectedMarker.setBackgroundResource(R.drawable.event_all_day_on);
                } else {
                    // View
                    // selectedMarker=itemView.findViewById(R.id.selected_marker);
                    // selectedMarker.setBackgroundResource(R.drawable.btn_check_off_normal_holo_dark);
                }
                /*** HCT_MODIFY end */
            } else {
                ...
                if (holder.selected) {
                    ...
                    // View
                    // selectedMarker=itemView.findViewById(R.id.selected_marker);
                    // selectedMarker.setBackgroundResource(R.drawable.event_all_day_off);
                }
                /*** HCT_MODIFY end */
            }
            holder.julianDay = row.mDay;
    ...
    public void selectItemByPosition(int position) {
        if (mRowInfo != null && position >= 0 && position < mRowInfo.size()) {
            RowInfo row = mRowInfo.get(position);
            if (row.mType == TYPE_MEETING) {
                mAgendaAdapter.selectItemByPosition(row.mPosition);
            }
        }
    }

    /*** HCT_MODIFY jianghejie */
    public void reSelectItemByPosition(int position) {
        if (mRowInfo != null && position >= 0 && position < mRowInfo.size()) {
            RowInfo row = mRowInfo.get(position);
            if (row.mType == TYPE_MEETING) {
                mAgendaAdapter.reSelectItemByPosition(row.mPosition);
            }
        }
    }
    ...
    public void selectAll() {
        mAgendaAdapter.selectAll();
    }

    public void cancelSelect() {
        mAgendaAdapter.cancelSelect();
    }
}
