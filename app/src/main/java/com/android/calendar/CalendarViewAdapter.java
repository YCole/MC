...
public class CalendarViewAdapter extends BaseAdapter {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (mShowDate) {
            ...
            if (convertView == null
                ...
                // as a convert view
                v.setTag(Integer
                        .valueOf(R.layout.actionbar_pulldown_menu_top_button));
            } else {
                v = convertView;
            }
            ...
            case ViewType.MONTH:
                ...
                break;
            case ViewType.WEEK:
                if (Utils.getIfShowWeekNumberInWeekView(mContext)) {
            ...
            if (convertView == null
                ...
                // as a convert view
                v.setTag(Integer
                        .valueOf(R.layout.actionbar_pulldown_menu_top_button_no_date));
            } else {
                v = convertView;
            }
    ...
    public void setTime(long time) {
        mMilliTime = time;
        if (mCallBack != null) {
            mCallBack.currentTime(mMilliTime);
        }
        notifyDataSetChanged();
    }
    ...
    public static OnTimeCurrentCallBack mCallBack;

    public static interface OnTimeCurrentCallBack {
        void currentTime(long time);
    }

    public static void setOnTimeCurrentCallBack(OnTimeCurrentCallBack callBack) {
        mCallBack = callBack;
    }
        ...
        if (julianDay == mTodayJulianDay) {
            dayOfWeek = mContext.getString(
                    ...
                    DateUtils.formatDateRange(mContext, mFormatter, mMilliTime,
                            mMilliTime, DateUtils.FORMAT_SHOW_WEEKDAY,
                            mTimeZone).toString());
    ...
    private String buildMonthDate() {
        ...
        String month[] = CalendarApplication.getContext().getResources()
                .getStringArray(R.array.month);
        String monthStr = CalendarApplication.getContext().getString(
                R.string.event_month);
        if (date.equals(month[0])) {
            date = "1";
        }
        if (date.equals(month[1])) {
            date = "2";
        }
        if (date.equals(month[2])) {
            date = "3";
        }
        if (date.equals(month[3])) {
            date = "4";
        }
        if (date.equals(month[4])) {
            date = "5";
        }
        if (date.equals(month[5])) {
            date = "6";
        }
        if (date.equals(month[6])) {
            date = "7";
        }
        if (date.equals(month[7])) {
            date = "8";
        }
        if (date.equals(month[8])) {
            date = "9";
        }
        if (date.equals(month[9])) {
            date = "10";
        }
        if (date.equals(month[10])) {
            date = "11";
        }
        if (date.equals(month[11])) {
            date = "12";
        }
    ...
    private String buildWeekDate() {
        // Calculate the start of the week, taking into account the "first day
        // of the week"
        // setting.

        Time t = new Time(mTimeZone);
        t.set(mMilliTime);
        t.hour = 0; // the week should be 7 days
        t.minute = 0;
        ...
        long weekEndTime = weekStartTime + DateUtils.WEEK_IN_MILLIS - 1;// -
    ...
    private String buildWeekNum() {
        ...
        int week = Utils.getWeekNumberFromCalendar(mMilliTime, mContext);
