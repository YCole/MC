...
public class DayView extends View implements View.OnCreateContextMenuListener,
    ...
    private static final int MAXJULIANDAY = 2465424;
    private static final int MAXWEEK = 3548;// 2037.12.31

    private static final int HOUR_WIDTH = 20;
    ...
    private static int DAY_HEAD_OFFSET;
    ...
    protected final static StringBuilder mStringBuilder = new StringBuilder(50);
    // TODO recreate formatter when locale changes
    protected final static Formatter mFormatter = new Formatter(mStringBuilder,
            Locale.getDefault());
    ...
    private static int AMPM_HEIGHT = 4;
    /**
    ...
    private static int HOURS_LEFT_MARGIN = 3;
    private static int HOURS_RIGHT_MARGIN = 3;
    ...
    private static float EVENT_TEXT_FONT_SIZE;
    ...
    private static int NEW_EVENT_HINT_FONT_SIZE;
    ...
    public DayView(Activity context, CalendarController controller,
        ...
        // AMPM_TEXT_SIZE = (int)
        // mResources.getDimension(R.dimen.ampm_text_size);
        ...
        DEFAULT_CELL_HEIGHT = (int) mResources
                .getDimension(R.dimen.list_height01);

        DAY_HEAD_OFFSET = (int) mResources
                .getDimension(R.dimen.day_head_offset);

        int eventTextSizeId;
        /*
         * if (mNumDays == 1) { eventTextSizeId =
         * R.dimen.day_view_event_text_size; } else { eventTextSizeId =
         * R.dimen.week_view_event_text_size; }
         */
        EVENT_TEXT_FONT_SIZE = (int) mResources
                .getDimension(R.dimen.event_text_font_size);
        ...
        mLongPressItems = new CharSequence[] {
                ...
                mResources.getString(R.string.copy_and_new_event_dialog_option),
    ...
    private void init(Context context) {
        ...
        mWeek_saturdayColor = mResources.getColor(R.color.cale_subtitle_color);
        mWeek_sundayColor = mResources.getColor(R.color.cale_subtitle_color);
        mCalendarDateBannerTextColor = mResources
                .getColor(R.color.color06_transparency10);
        mFutureBgColorRes = mResources.getColor(R.color.color02_transparency14);
        ...
        mCalendarHourLabelColor = mResources
                .getColor(R.color.color06_transparency14);
         ...
         * computeMaxStringWidth(mHoursWidth, ampm, p) + HOURS_RIGHT_MARGIN);
         * mHoursWidth = Math.max(MIN_HOURS_WIDTH, mHoursWidth);
         */
        mHoursWidth = HOUR_WIDTH;
        mHoursWidth *= mScale;
    ...
    private void recalc() {
        ...
        if (start < 0) {
            mFirstJulianDay = Time.getJulianDay(start, mBaseDate.gmtoff) - 1;
        } else {
        ...
        if (dayChanged
            ...
            if (dayChanged || hourChanged || speakEvents) {
                ...
                try {
                    ...
                    Log.e(TAG, "Accessibility Error: " + ex);
                }
            }
        }
    }
    ...
    private void drawScrollLine(Rect r, Canvas canvas, Paint p) {
        ...
        final int left = computeDayLeftPosition(mNumDays - 1);
    ...
    private void drawAmPm(Canvas canvas, Paint p) {
        ...
        int y = mFirstCell + mFirstHourOffset + 2 * mHoursTextHeight + HOUR_GAP
                - 2 * AMPM_HEIGHT;
        canvas.drawText(text, mHoursWidth / 2, y, p);
    ...
    private void drawCurrentTimeLine(Rect r, final int day, final int top,
        ...
        p.setStrokeWidth(ALLDAY_TOP_MARGIN);
    ...
    private void drawSelectedRect(Rect r, Canvas canvas, Paint p) {
        ...
        if (mSelectionMode != SELECTION_HIDDEN && !mSelectionAllday) {
            ...
            r.left = computeDayLeftPosition(daynum) + EVENT_RECT_LEFT_MARGIN
                    / 3;
            r.right = computeDayLeftPosition(daynum + 1)
                    + EVENT_RECT_LEFT_MARGIN / 3;
            ...
            if (mNumDays > 1) {
                ...
                p.setAntiAlias(true);
                p.setColor(mContext.getResources().getColor(
                        R.color.calendar_event_text_color));
    ...
    private void drawHours(Rect r, Canvas canvas, Paint p) {
        setupHourTextPaint(p);
        int y = HOUR_GAP + mHoursTextHeight - 2 * AMPM_HEIGHT;
    ...
    private void drawDayHeader(String dayStr, int day, int cell, Canvas canvas,
        ...
        if (mNumDays > 1) {
            ...
            // Draw day of the month
            x = computeDayLeftPosition(day + 1);
            p.setTextAlign(Align.CENTER);
            p.setTextSize(DATE_HEADER_FONT_SIZE);

            // p.setTypeface(todayIndex == day ? mBold : Typeface.DEFAULT);
            p.setTypeface(mBold);
            String dateStr = dayStr + " " + dateNumStr;
            float width = p.measureText(dateStr);
            float dayWidth = (mViewWidth - mHoursWidth) / (mNumDays);
            canvas.drawText(dateStr, x - ((dayWidth) / 2), y - DAY_HEAD_OFFSET,
                    p);
            /*
             * // Draw day of the week x -= p.measureText(" " + dateNumStr);
             * p.setTextAlign(Align.CENTER);
             * p.setTextSize(DAY_HEADER_FONT_SIZE);
             * p.setTypeface(Typeface.DEFAULT_BOLD); canvas.drawText(dayStr,
             * x-5, y-20, p);
             */
        } else {
            ...
            p.setTextSize(DATE_HEADER_FONT_SIZE);
            p.setTypeface(todayIndex == day ? mBold : Typeface.DEFAULT_BOLD);
    ...
    private void drawGridBackground(Rect r, Canvas canvas, Paint p) {
        ...
        for (int hour = 0; hour <= 24; hour++) {
            // mLines[linesIndex++] = GRID_LINE_LEFT_MARGIN + 6 * AMPM_HEIGHT;
    ...
    private void drawAllDayEvents(int firstDay, int numDays, Canvas canvas,
        ...
        for (int i = 0; i < numEvents; i++) {
            ...
            // event rectangle.
            event.left = computeDayLeftPosition(startIndex) - 1;
    ...
    private Rect drawEventRect(Event event, Canvas canvas, Paint p,
        ...
        r.bottom = Math.min((int) event.bottom - EVENT_RECT_BOTTOM_MARGIN / 3,
                visibleBot);
        r.left = (int) event.left;
        r.right = (int) event.right + EVENT_RECT_TOP_MARGIN / 3;
        ...
        r.top = Math.max((int) event.top + floorHalfStroke, visibleTop);
        r.bottom = Math.min((int) event.bottom - EVENT_RECT_BOTTOM_MARGIN / 3
                - ceilHalfStroke, visibleBot);
        if (mNumDays == 1) {
            r.left += floorHalfStroke + EVENT_RECT_LEFT_MARGIN * 3;
        } else {
        ...
        Rect rect = new Rect();
        rect.top = Math.max((int) event.top, visibleTop);
        rect.bottom = Math.min((int) event.bottom - EVENT_RECT_BOTTOM_MARGIN
                / 3, visibleBot);
        rect.left = (int) event.left;
        rect.right = rect.left + EVENT_RECT_LEFT_MARGIN * 3;
        Paint p2 = new Paint();
        p2.setStrokeWidth(EVENT_RECT_STROKE_WIDTH);
        p2.setColor(color);
        int alpha2 = p2.getAlpha();
        p2.setAlpha(mEventsAlpha);
        ...
        r.bottom = (int) event.bottom - EVENT_RECT_BOTTOM_MARGIN / 3;
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        case MotionEvent.ACTION_MOVE:
            ...
            x_move = ev.getX();
            y_move = ev.getY();
            boolean isLeft = ((x_move - x_down) > 0);
            /*
             * if (mNumDays==1){ if(!isLeft||mSelectionDay >
             * Time.EPOCH_JULIAN_DAY){ mGestureDetector.onTouchEvent(ev); }
             * }else{ if(!isLeft||mPosition > 0){
             * mGestureDetector.onTouchEvent(ev); } }
             */
            mGestureDetector.onTouchEvent(ev);
            return true;
        ...
        case MotionEvent.ACTION_UP:
            ...
            x_up = ev.getX();
            y_up = ev.getY();
            boolean isPrevious = ((x_up - x_down) > 0);
            boolean isNext = ((x_up - x_down) < 0);
            ...
            if ((mTouchMode & TOUCH_MODE_HSCROLL) != 0) {
                ...
                if (Math.abs(mViewStartX) > mHorizontalSnapBackThreshold) {
                    ...
                    if (mNumDays == 1) {
                        if (((!isPrevious && mSelectionDay < MAXJULIANDAY))
                                || (isPrevious && mSelectionDay > Time.EPOCH_JULIAN_DAY)) {
                            switchViews(mViewStartX > 0, mViewStartX,
                                    mViewWidth, 0);
                        }
                    } else {
                        if ((!isPrevious && mPosition < MAXWEEK)
                                || (isPrevious && mPosition > 0)) {
                            switchViews(mViewStartX > 0, mViewStartX,
                                    mViewWidth, 0);
                        }
                    }
                    ...
                    // if(!isPrevious||mSelectionDay > Time.EPOCH_JULIAN_DAY){
                    recalc();
                    invalidate();
                    mViewStartX = 0;
                    // }
    ...
    private void setSelectedDay(int d) {
        if (d >= Time.EPOCH_JULIAN_DAY && d <= MAXJULIANDAY) {
            mSelectionDay = d;
            mSelectionDayForAccessibility = d;
            mPosition = Utils.getWeeksSinceEpochFromJulianDay(mSelectionDay,
                    Utils.getFirstDayOfWeek(getContext()));
        }
    }
    ...
    class CalendarGestureListener extends
        ...
        public boolean onScroll(MotionEvent e1, MotionEvent e2,
            ...
            boolean isPrevious = (e2.getRawX() - e1.getRawX()) > 0;
            if (mNumDays == 1) {
                if (((!isPrevious && mSelectionDay < MAXJULIANDAY))
                        || (isPrevious && mSelectionDay > Time.EPOCH_JULIAN_DAY)) {
            ...
            /*
             * if (mSelectionDay > Time.EPOCH_JULIAN_DAY || (e2.getRawX() -
             * e1.getRawX()) < 0){ DayView.this.doScroll(e1, e2, distanceX,
             * distanceY); }
             */
            return true;
        }
        ...
        public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX,
            ...
            /*
             * if (mSelectionDay > Time.EPOCH_JULIAN_DAY || (e2.getRawX() -
             * e1.getRawX()) < 0){ DayView.this.doFling(e1, e2, velocityX,
             * velocityY); }
             */
            return true;
        }
    ...
    public boolean onLongClick(View v) {
        ...
        if (mSelectedEvent != null) {
            String selectedEventTitle = (String) mSelectedEvent.title;
            if (selectedEventTitle != null && !selectedEventTitle.equals("")) {
                mLongPressTitle = selectedEventTitle;
            }
            mLongPressItems = new CharSequence[] {
                    mResources.getString(R.string.edit_label),
                    mResources
                            ...
                            public void onClick(DialogInterface dialog,
                                ...
                                if (which == 1) {
                                    ...
                                    DeleteEventHelper deleteHelper = new DeleteEventHelper(
                                            mContext, mContext, false);
                                    deleteHelper.delete(begin, end, id, which);
                                } else if (which == 0) {
                                    /**
                                     * Uri uri =
                                     * ContentUris.withAppendedId(Events
                                     * .CONTENT_URI, mSelectedEvent.id); Intent
                                     ...
                                     * intent.setClass(mContext,
                                     * EditEventActivity.class);
                                     * intent.putExtra(EVENT_EDIT_ON_LAUNCH,
                                     * true); mContext.startActivity(intent);
                                     */
                                    if (mSelectedEvent != null) {
                                        Log.d(TAG, "lxg mSelectedEvent.id="
                                                + mSelectedEvent.id);
                                        Uri uri = ContentUris.withAppendedId(
                                                Events.CONTENT_URI,
                                                mSelectedEvent.id);
