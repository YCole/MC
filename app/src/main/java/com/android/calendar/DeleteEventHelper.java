 ...
 * deletion, then this pops up a confirmation dialog. If the user confirms, then
 * the normal event is deleted.
 * 
 * <p>
 * If a repeating event is selected for deletion, then this pops up dialog
 * asking if the user wants to delete just this one instance, or all the events
 * in the series, or this event plus all following events. The user may also
 * cancel the delete.
 * </p>
 * 
 * <p>
 * To use this class, create an instance, passing in the parent activity and a
 * boolean that determines if the parent activity should exit if the event is
 * deleted. Then to use the instance, call one of the {@link delete()} methods
 * on this class.
 * 
...
public class DeleteEventHelper {
    ...
    public DeleteEventHelper(Context context, Activity parentActivity,
        ...
        mService = new AsyncQueryService(mContext) {
            ...
            protected void onQueryComplete(int token, Object cookie,
                ...
                cursor.close();
                DeleteEventHelper.this.delete(mStartMillis, mEndMillis, mModel,
                        mWhichDelete);
            }

            @Override
            protected void onInsertComplete(int token, Object cookie, Uri uri) {
                deleteAlarm(mModel.mId);
            }
            ...
            protected void onUpdateComplete(int token, Object cookie, int result) {
                ...
                if (mContext instanceof AllInOneActivity) {
                    ((AllInOneActivity) mContext).refreshAgendaFragment();
                }
            }
            ...
            protected void onDeleteComplete(int token, Object cookie, int result) {
                AlertService.updateAlertNotification(mContext);
                if (mModel.mEventType == EventItem.VIEW_EVENT_LOCATION) {
                    // delete Fence
                    try {
                        LbsServiceHelper.deleteLocationFence(mContext,
                                Integer.parseInt(mModel.mActionData2),
                                mModel.mId);
    ...
    private void deleteAlarm(long eventId) {
        String selection = CalendarAlerts.EVENT_ID + "=" + eventId;
        mService.startDelete(mService.getNextToken(), null,
                CalendarAlerts.CONTENT_URI, selection, null, Utils.UNDO_DELAY);
    }
    ...
    /**
     * Does the required processing for deleting an event, which includes first
     * popping up a dialog asking for confirmation (if the event is a normal
     * event) or a dialog asking which events to delete (if the event is a
     * repeating event). The "which" parameter is used to check the initial
     * selection and is only used for repeating events. Set "which" to -1 to
     ...
     *            the event id
     * @param which
     *            one of the values {@link DELETE_SELECTED},
     ...
     * {@link CalendarEventModel} object, which must have a valid uri for
     * referencing the event in the database and have the required fields listed
     * below. The required fields for a normal event are:
     * 
     * <ul>
     * <li>Events._ID</li>
     * <li>Events.TITLE</li>
     ...
     * </ul>
     * 
     * If the event no longer exists in the db this will still prompt the user
     * but will return without modifying the db after the query returns.
     * 
     * @param begin
     *            the begin time of the event, in UTC milliseconds
     * @param end
     *            the end time of the event, in UTC milliseconds
     * @param cursor
     *            the database cursor containing the required fields
     * @param which
     *            one of the values {@link DELETE_SELECTED},
     *            {@link DELETE_ALL_FOLLOWING}, {@link DELETE_ALL}, or -1
     */
    public void delete(long begin, long end, CalendarEventModel model, int which) {
        ...
        if (TextUtils.isEmpty(rRule)) {
            ...
            int[] labelValues = res
                    .getIntArray(R.array.delete_repeating_values);
            ...
            if (labelIndex.contains(which)) {
                // transform the which to the index in the array
                which = labelIndex.indexOf(which);
            } else {
                which = labelIndex.indexOf(DELETE_ALL);
            }
            mWhichIndex = labelIndex;
            if (which > -1 && which < mWhichIndex.size()) {
                mWhichDelete = mWhichIndex.get(which);
            }
            ArrayAdapter<String> adapter = new ArrayAdapter<String>(mContext,
                    R.layout.select_dialog_singlechoice_material, labelArray);
            AlertDialog dialog = new AlertDialog.Builder(mContext)
                    .setTitle(
                            mContext.getString(
                                    R.string.delete_recurring_event_title,
                                    model.mTitle))
