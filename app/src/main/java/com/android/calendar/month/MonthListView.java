...
public class MonthListView extends ListView {
    ...
    private final Runnable mTimezoneUpdater = new Runnable() {
        ...
        public void run() {
            if (mTempTime != null && mListContext != null) {
                mTempTime.timezone = Utils.getTimeZone(mListContext,
                        mTimezoneUpdater);
            }
        }
    };
    ...
    private void doFling(float velocityY) {
        ...
        MotionEvent cancelEvent = MotionEvent.obtain(mDownActionTime,
        ...
        int viewsToFling = (scrollToDay - day) / 7
                - ((monthsToJump <= 0) ? 1 : 0);
        int offset = (viewsToFling > 0) ? -(firstViewHeight
    ...
    private int getUpperRightJulianDay() {
        ...
        return child.getFirstJulianDay()
                + SimpleDayPickerFragment.DAYS_PER_WEEK - 1;
    }
}
