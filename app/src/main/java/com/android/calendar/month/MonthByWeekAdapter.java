...
public class MonthByWeekAdapter extends SimpleWeeksAdapter {
    ...
    private float monthItemHeight = 65;
    ...
    public MonthByWeekAdapter(Context context, HashMap<String, Integer> params,
        ...
        Settings.System.putLong(resolver, "calendar_touch_day", 0);
        // HCT_MODIFY lixiange MF3.0
    }
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        int viewParamsHeight = dpToPx(mContext, monthItemHeight);
        drawingParams.put(SimpleWeekView.VIEW_PARAMS_HEIGHT, viewParamsHeight);
    ...
    public boolean onTouch(View v, MotionEvent event) {
        ...
        // Event was tapped - switch to the detailed view making sure the click
        // animation
        // is done first.
        if (mGestureDetector.onTouchEvent(event)) {
            // HCT_MODIFY lixiange MF3.0 add liblight day and display day event
            // when click day
            Time mTouchday = ((SimpleWeekView) v).getDayFromLocation(event
                    .getX());
            int touchDay = Time.getJulianDay(mTouchday.toMillis(true),
                    mTouchday.gmtoff);
            if (DEBUG) {
                Long tmp = System.currentTimeMillis() - mClickTime;
                Log.d(TAG, "touch, params=" + touchDay + ", " + lastIndex
                        + ", " + tmp);
            }
            if ((lastIndex == -1) || (touchDay != lastIndex)) {
                lastIndex = touchDay;
                mClickTime = System.currentTimeMillis();
            } else if (lastIndex == touchDay) {
                mSingleTapUpView = (MonthWeekEventsView) v;
                long delay = System.currentTimeMillis() - mClickTime;
                // Make sure the animation is visible for at least mOnTapDelay -
                // mOnDownDelay ms
                if (delay < mTotalClickDelay * 5) {
                    mListView.postDelayed(mDoSingleTapUp,
                            delay > mTotalClickDelay ? 0 : mTotalClickDelay
                                    - delay);
                }
                // lastIndex = -1;
                mClickTime = System.currentTimeMillis();
            ...
            // Animate a click - on down: show the selected day in the "clicked"
            // color.
            ...
            case MotionEvent.ACTION_DOWN:
                ...
                for (int i = 0; i < 6; i++) {
                    child = (MonthWeekEventsView) mListView.getChildAt(i);
                    if (child != null)
                        child.clearClickedDay();
                }
                // HCT_MODIFY lixiange MF3.0 add liblight day and display day
                ...
                Settings.System.putLong(resolver, "calendar_touch_day",
                        mTouchday.toMillis(true));
                if (DEBUG) {
                    Log.d(TAG, "onTouch, set calendar_touch_day= " + mTouchday);
                }
                mController.setTime(mTouchday.toMillis(false));
                // HCT_MODIFY lixiange MF3.0 add liblight day and display day
                // event when click day
                break;
            ...
            case MotionEvent.ACTION_MOVE:
                // No need to cancel on vertical movement, ACTION_SCROLL will do
                // that.
        ...
        // Do not tell the frameworks we consumed the touch action so that fling
        // actions can be
     ...
     *           Log.d(TAG,"lxg onTouch mDoClick ===============");
     *           mClickedView.setClickedDay(mClickedXLocation); }
