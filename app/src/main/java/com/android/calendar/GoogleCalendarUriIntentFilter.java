...
public class GoogleCalendarUriIntentFilter extends Activity {
     ...
     * 
     * The URI contains an "eid" parameter, which is comprised of an ID,
     * followed by a space, followed by the calendar email address. The domain
     * is sometimes shortened. See the switch statement. This is Base64-encoded
    ...
    private String[] extractEidAndEmail(Uri uri) {
        try {
            ...
            if (DEBUG)
                Log.d(TAG, "decoded eid=" + new String(decodedBytes));
            ...
            for (int spacePosn = 0; spacePosn < decodedBytes.length; spacePosn++) {
                if (decodedBytes[spacePosn] == ' ') {
                    ...
                    String email = new String(decodedBytes, spacePosn + 1,
                            emailLen);
                    if (DEBUG)
                        Log.d(TAG, "eid=   " + eid);
                    if (DEBUG)
        ...
        if (intent != null) {
            ...
            if (uri != null) {
                ...
                if (eidParts == null) {
                    ...
                    final String selection = Events._SYNC_ID + " LIKE \"%"
                            + syncId + "\" AND " + Calendars.OWNER_ACCOUNT
                            + " LIKE \"" + ownerAccount + "\"";

                    if (DEBUG)
                        Log.d(TAG, "selection: " + selection);
                    Cursor eventCursor = getContentResolver().query(
                            Events.CONTENT_URI, EVENT_PROJECTION, selection,
                            null, Calendars.CALENDAR_ACCESS_LEVEL + " desc");
                    if (DEBUG)
                        Log.d(TAG, "Found: " + eventCursor.getCount());

                    if (eventCursor == null || eventCursor.getCount() == 0) {
                        Log.i(TAG, "NOTE: found no matches on event with id='"
                                + syncId + "'");
                        return;
                    }
                    ...
                    try {
                        ...
                        while (eventCursor.moveToNext()) {
                            ...
                            long startMillis = eventCursor
                                    .getLong(EVENT_INDEX_START);
                            long endMillis = eventCursor
                                    .getLong(EVENT_INDEX_END);
                            if (DEBUG)
                                Log.d(TAG,
                                        "_id: "
                                                + eventCursor
                                                        .getLong(EVENT_INDEX_ID));
                            if (DEBUG)
                            ...
                            if (endMillis == 0) {
                                String duration = eventCursor
                                        .getString(EVENT_INDEX_DURATION);
                                if (DEBUG)
                            ...
                            if ("RESPOND".equals(uri
                                    .getQueryParameter("action"))) {
                                try {
                                    switch (Integer.parseInt(uri
                                            .getQueryParameter("rst"))) {
                                    case 1: // Yes
                            ...
                            if (attendeeStatus == Attendees.ATTENDEE_STATUS_NONE) {
                                ...
                                updateSelfAttendeeStatus(eventId, ownerAccount,
                                        attendeeStatus, intent);
                            }
                            finish();
                            return;
                        }
    ...
    private void updateSelfAttendeeStatus(int eventId, String ownerAccount,
            final int status, final Intent intent) {
        ...
        final AsyncQueryHandler queryHandler = new AsyncQueryHandler(cr) {
            ...
            protected void onUpdateComplete(int token, Object cookie, int result) {
                ...
                Toast.makeText(GoogleCalendarUriIntentFilter.this, toastId,
                        Toast.LENGTH_LONG).show();
            }
        };
        final ContentValues values = new ContentValues();
        values.put(Attendees.ATTENDEE_STATUS, status);
        queryHandler.startUpdate(0, null, Attendees.CONTENT_URI, values,
