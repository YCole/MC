...
public class SelectCalendarsSimpleAdapter extends BaseAdapter implements
        ListAdapter, OnCalendarColorsLoadedListener {
    ...
    public SelectCalendarsSimpleAdapter(Context context, int layout, Cursor c,
        ...
        mColorCalendarSecondaryVisible = mRes
                .getColor(R.color.calendar_secondary_visible);
        mColorCalendarSecondaryHidden = mRes
                .getColor(R.color.calendar_secondary_hidden);
        ...
        mFragmentManager = fm;
        mColorPickerDialog = (CalendarColorPickerDialog) fm
    ...
    private static class TabletCalendarItemBackgrounds {
        ...
        static int[] getBackgrounds() {
            ...
            mBackgrounds[IS_SELECTED | IS_BOTTOM] = R.drawable.calname_bottom_select_underunselected;

            mBackgrounds[IS_SELECTED | IS_BOTTOM | IS_BELOW_SELECTED] = R.drawable.calname_bottom_select_underselect;
            mBackgrounds[IS_SELECTED | IS_TOP | IS_BOTTOM | IS_BELOW_SELECTED] = mBackgrounds[IS_SELECTED
                    | IS_BOTTOM | IS_BELOW_SELECTED];
            mBackgrounds[IS_SELECTED | IS_TOP | IS_BOTTOM] = mBackgrounds[IS_SELECTED
                    | IS_BOTTOM | IS_BELOW_SELECTED];
            ...
            mBackgrounds[IS_BOTTOM | IS_BELOW_SELECTED] = R.drawable.calname_bottom_unselected_underselect;
    ...
    private void initData(Cursor c) {
        ...
        // TODO create a broadcast listener for ACTION_PROVIDER_CHANGED to
        // update the cursor
        mCursor = c;
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        if (convertView == null) {
            ...
            /**
             * final View delegate = view.findViewById(R.id.color); final View
             * delegateParent = (View) delegate.getParent();
             * delegateParent.post(new Runnable() {
             * 
             * @Override public void run() { final Rect r = new Rect();
             *           delegate.getHitRect(r); r.top -=
        ...
        calendarName.setTextColor(mRes.getColor(R.color.text_color));
        calendarName.setText(name);

        // HCT_MODIFY lixiange MF3.0 add My Calendar
        if ("My calendar".equals(name)) {
            calendarName.setText(mRes.getString(R.string.my_calendar));
        }
        // HCT_MODIFY lixiange MF3.0 add My Calendar

        /**
         * View colorView = view.findViewById(R.id.color);
         * colorView.setBackgroundColor(color); colorView.setOnClickListener(new
         * OnClickListener() {
         * 
         * @Override public void onClick(View v) { // Purely for sanity
         *           check--view should be disabled if account has no more
         *           colors if (!hasMoreColors(position)) { return; }
         * 
        ...
        /*
         * int textColor; if (selected) { textColor = mColorCalendarVisible; }
         * else { textColor = mColorCalendarHidden; }
        ...
        if (syncCheckBox != null) {
            ...
            if (!TextUtils.isEmpty(mData[position].accountName)) {
                // &&
                // !mData[position].accountName.endsWith("calendar.google.com"))
                // {
                // && !mData[position].ownerAccount.equals(name)
                /*
                 * int secondaryColor; if (selected) { secondaryColor =
                 * mColorCalendarSecondaryVisible; } else { secondaryColor =
                 * mColorCalendarSecondaryHidden; }
                 */
                secondaryText.setText(mData[position].accountName);
                secondaryText.setTextColor(mRes
            ...
            CheckBox visibleCheckBox = (CheckBox) view
                    .findViewById(R.id.visible_check_box);
    ...
    private boolean hasMoreColors(int position) {
        return mCache.hasColors(mData[position].accountName,
                mData[position].accountType);
    }
    ...
    protected Drawable getBackground(int position, boolean selected) {
        ...
        bg |= (position == 0 && mOrientation == Configuration.ORIENTATION_LANDSCAPE) ? IS_TOP
                : 0;
        bg |= position == mData.length - 1 ? IS_BOTTOM : 0;
        bg |= (position > 0 && mData[position - 1].selected) ? IS_BELOW_SELECTED
                : 0;
        return mRes
                .getDrawable(TabletCalendarItemBackgrounds.getBackgrounds()[bg]);
    }
