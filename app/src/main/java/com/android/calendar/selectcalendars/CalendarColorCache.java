 ...
 * (name and type) of the accounts which contain optional calendar colors, and
 * thus should allow for the user to choose calendar colors.
 */
public class CalendarColorCache {
    ...
    private static String[] PROJECTION = new String[] { Colors.ACCOUNT_NAME,
            Colors.ACCOUNT_TYPE };

    /**
     * Interface which provides callback after provider query of calendar
     * colors.
     */
    public interface OnCalendarColorsLoadedListener {

        /**
         * Callback after the set of accounts with additional calendar colors
         * are loaded.
         */
        void onCalendarColorsLoaded();
    }

    public CalendarColorCache(Context context,
            OnCalendarColorsLoadedListener listener) {
        mListener = listener;
    ...
    /**
     * Does a set lookup to determine if a specified account has more optional
     * calendar colors.
     */
    ...
    /**
     * Generates a single key based on account name and account type for map
     * lookup/insertion.
     */
    private String generateKey(String accountName, String accountType) {
        mStringBuffer.setLength(0);
        return mStringBuffer.append(accountName).append(SEPARATOR)
