...
import com.hct.calendar.almanac.LunarCalendar;
...
public class Utils {
    ...
    public static final String INTENT_KEY_MONTH_VIEW = "MONTH_VIEW";
     ...
     * 19.49",  W 122° 5' 4.47"
     **/
    private static final String COORD_DEGREES_LATITUDE = "([-+NnSs]"
            + "(\\s)*)?" + "[1-9]?[0-9](\u00B0)" + "(\\s)*"
            + "([1-5]?[0-9]\')?" + "(\\s)*" + "([1-5]?[0-9]"
            + "(\\.[0-9]+)?\")?" + "((\\s)*" + "[NnSs])?";
    private static final String COORD_DEGREES_LONGITUDE = "([-+EeWw]"
            + "(\\s)*)?" + "(1)?[0-9]?[0-9](\u00B0)" + "(\\s)*"
            + "([1-5]?[0-9]\')?" + "(\\s)*" + "([1-5]?[0-9]"
            + "(\\.[0-9]+)?\")?" + "((\\s)*" + "[EeWw])?";
    private static final String COORD_DEGREES_PATTERN = COORD_DEGREES_LATITUDE
            + "(\\s)*" + "," + "(\\s)*" + COORD_DEGREES_LONGITUDE;
    private static final String COORD_DECIMAL_LATITUDE = "[+-]?"
            + "[1-9]?[0-9]" + "(\\.[0-9]+)" + "(\u00B0)?";
    private static final String COORD_DECIMAL_LONGITUDE = "[+-]?"
            + "(1)?[0-9]?[0-9]" + "(\\.[0-9]+)" + "(\u00B0)?";
    private static final String COORD_DECIMAL_PATTERN = COORD_DECIMAL_LATITUDE
            + "(\\s)*" + "," + "(\\s)*" + COORD_DECIMAL_LONGITUDE;
    private static final Pattern COORD_PATTERN = Pattern
            .compile(COORD_DEGREES_PATTERN + "|" + COORD_DECIMAL_PATTERN);
    ...
    public static int getViewTypeFromIntentAndSharedPref(Activity activity) {
        ...
        if (extras != null) {
            ...
            } else if (INTENT_VALUE_VIEW_TYPE_DAY.equals(extras
                    .getString(INTENT_KEY_VIEW_TYPE))) {
                // Not sure who uses this. This logic came from LaunchActivity
                return ViewType.DAY;
            } else if (extras.getBoolean(INTENT_KEY_MONTH_VIEW, false)) {
                return ViewType.MONTH;
            }
        }
     ...
     * SharedPreferences prefs) { boolean vibrate; if
     * (prefs.contains(KEY_ALERTS_VIBRATE_WHEN)) { // Migrate setting to new 4.2
     * behavior // // silent and never -> off // always -> on String vibrateWhen
     * = prefs.getString(KEY_ALERTS_VIBRATE_WHEN, null); vibrate = vibrateWhen
     * != null && vibrateWhen.equals(context
     * .getString(R.string.prefDefault_alerts_vibrate_true));
     * prefs.edit().remove(KEY_ALERTS_VIBRATE_WHEN).commit(); Log.d(TAG,
     * "Migrating KEY_ALERTS_VIBRATE_WHEN(" + vibrateWhen +
     * ") to KEY_ALERTS_VIBRATE = " + vibrate); } else { vibrate =
     * prefs.getBoolean(GeneralPreferences.KEY_ALERTS_VIBRATE, false); } return
     * vibrate; }
     */
    ...
    public static String getRingTonePreference(Context context) {
        SharedPreferences prefs = context.getSharedPreferences(
                GeneralPreferences.SHARED_PREFS_NAME_NO_BACKUP,
        ...
        if (ringtone == null) {
            ...
            ringtone = getSharedPreference(context,
    ...
    public static void setRingTonePreference(Context context, String value) {
        SharedPreferences prefs = context.getSharedPreferences(
                GeneralPreferences.SHARED_PREFS_NAME_NO_BACKUP,
                Context.MODE_PRIVATE);
        prefs.edit().putString(GeneralPreferences.KEY_ALERTS_RINGTONE, value)
                .apply();
    }
    ...
    static void setDefaultView(Context context, int viewId) {
        ...
        if (mAllowWeekForDetailView
                && viewId == CalendarController.ViewType.WEEK) {
    ...
    static void setStartView(Context context, int viewId) {
        SharedPreferences prefs = GeneralPreferences
                .getSharedPreferences(context);
        SharedPreferences.Editor editor = prefs.edit();
        // Record the (new) start view
    ...
    public static final long timeFromIntentInMillis(Intent intent) {
        return timeFromIntentInMillis(intent, false);
    }
     ...
     * @return
     */
    public static Date strToDate(String strDate, String formart) {
    ...
    public static int getWeekNumsByTime(Time t, int firstDayOfweek) {
        return getValueFromCalendarByField(t, firstDayOfweek,
                Calendar.WEEK_OF_MONTH);
    }

    public static int getDayInWeekIndex(Time t, int firstDayOfweek) {
        int weekDay = getValueFromCalendarByField(t, firstDayOfweek,
                Calendar.DAY_OF_WEEK);
        int weekStart = convertDayOfWeekFromTimeToCalendar(firstDayOfweek);
        if (Calendar.SUNDAY != weekStart) {
            int diff = 0;
    ...
    public static int getValueFromCalendarByField(Time t, int firstDayOfweek,
            int field) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone(t.timezone));
        calendar.set(t.year, t.month, t.monthDay);
        calendar.setFirstDayOfWeek(convertDayOfWeekFromTimeToCalendar(firstDayOfweek));
        return calendar.get(field);
    }

    public static int getWeekOfMonthNumsByTime(Time t, int firstDayOfweek) {
        Calendar calendar = Calendar.getInstance();
        calendar.setTimeZone(TimeZone.getTimeZone(t.timezone));
        calendar.set(t.year, t.month, t.monthDay);
        calendar.setFirstDayOfWeek(convertDayOfWeekFromTimeToCalendar(firstDayOfweek));
    ...
    public static int getFirstJulianFromWeeksSinceEpoch(int week,
            int firstDayOfweek) {
        int julianMonday = getJulianMondayFromWeeksSinceEpoch(week);
        int weekDay = Time.MONDAY;
        int offset = weekDay - firstDayOfweek;
        if (weekDay < firstDayOfweek) {
            offset = offset + 7;
        }
        return julianMonday - offset;
    }
    ...
    public static int getFirstDayOfWeek(Context context) {
        ...
        // GeneralPreferences.KEY_WEEK_START_DAY,
        // GeneralPreferences.WEEK_START_DEFAULT);
        SharedPreferences sp = context.getSharedPreferences(
                CalendarSettings.SETTINGS_SP_NAME, Context.MODE_PRIVATE);
        String weekStrs[] = context.getResources().getStringArray(
                R.array.preferences_week_start_day_values);
        String pref = weekStrs[sp.getInt("preferences_week_start_day", 0)];
        int startDay;
        ...
        Log.d("liaozhenbin", "startDay: " + startDay + "---pref: " + pref);
    ...
    public static boolean getShowWeekNumber(Context context) {
        final SharedPreferences prefs = GeneralPreferences
                .getSharedPreferences(context);
        boolean isFirstLoad = prefs.getBoolean(
                GeneralPreferences.KEY_FIRST_LOAD, true);
        boolean showWeek = false;
        if (isFirstLoad) {
            setSharedPreference(context, GeneralPreferences.KEY_FIRST_LOAD,
                    false);
            showWeek = isAbroadBranch(context);
            setSharedPreference(context, GeneralPreferences.KEY_SHOW_WEEK_NUM,
                    showWeek);
        } else {
            showWeek = prefs.getBoolean(GeneralPreferences.KEY_SHOW_WEEK_NUM,
                    GeneralPreferences.DEFAULT_SHOW_WEEK_NUM);
        }
        return showWeek;
    }

    public static boolean getIfShowWeekNumberInWeekView(Context context) {
        return getShowWeekNumber(context);
    }

    public static boolean isEnableAlmanacShow(Context context) {
        if (!isAbroadBranch(context)
                && compareWithCurLang(context, Utils.SIMPLIFIED_CHINESE)
                && getSharedPreference(context,
                        GeneralPreferences.KEY_SUBSCRIBE_ALMANAC, false)) {
            return true;
        }
        return false;
    }

    public static boolean isEnableLunarRecurrence(Context context) {
        return Utils.compareWithCurLang(context, Utils.SIMPLIFIED_CHINESE)
                && !Utils.isAbroadBranch(context);
    }

    public static boolean isEnableWeatherShow(Context context) {
        if (compareWithCurLang(context, Utils.SIMPLIFIED_CHINESE)
                && getSharedPreference(context,
                        GeneralPreferences.KEY_SUBSCRIBE_WEATHER, false)) {
            return true;
        }
        return false;
    }
     ...
     */
    public static long convertAlldayUtcToLocal(Time recycle, long utcTime,
            String tz) {
        if (recycle == null) {
            recycle = new Time();
        }
    ...
    public static long convertAlldayLocalToUTC(Time recycle, long localTime,
            String tz) {
        if (recycle == null) {
            recycle = new Time();
        }
    ...
    public static int getDisplayColorFromColor(int color) {
        ...
        return color;
        /*
         * float[] hsv = new float[3]; Color.colorToHSV(color, hsv); hsv[1] =
         * Math.min(hsv[1] * SATURATION_ADJUST, 1.0f); hsv[2] = hsv[2] *
         * INTENSITY_ADJUST; return Color.HSVToColor(hsv);
         */
    }
    ...
    public static void trySyncAndDisableUpgradeReceiver(Context context) {
        ...
        ComponentName upgradeComponent = new ComponentName(context,
                UpgradeReceiver.class);
        ...
        ContentResolver.requestSync(null /* no account */,
     ...
     * @return
     */
    public static HashMap<Integer, DNAStrand> createDNAStrands(
            int firstJulianDay, ArrayList<Event> events, int top, int bottom,
            int minPixels, int[] dayXs, Context context) {

        if (!mMinutesLoaded) {
            if (context == null) {
                Log.wtf(TAG,
                        "No context and haven't loaded parameters yet! Can't create DNA.");
                return null;
            }
            Resources res = context.getResources();
            CONFLICT_COLOR = res
                    .getColor(R.color.month_dna_conflict_time_color);
            WORK_DAY_START_MINUTES = res
                    .getInteger(R.integer.work_start_minutes);
        ...
        if (events == null || events.isEmpty() || dayXs == null
                || dayXs.length < 1 || bottom - top < 8 || minPixels < 0) {
            Log.e(TAG, "Bad values for createDNAStrands! events:" + events
                    + " dayXs:" + Arrays.toString(dayXs) + " bot-top:"
                    + (bottom - top) + " minPixels:" + minPixels);
            return null;
        }
        ...
        for (Event currEvent : events) {
            ...
            if (currEvent.endDay < firstJulianDay
                    || currEvent.startDay > lastJulianDay) {
                continue;
            }
            if (currEvent.drawAsAllday()) {
                addAllDayToStrands(currEvent, strands, firstJulianDay,
                        dayXs.length);
                continue;
            }
            ...
            if (event.startDay == event.endDay
                ...
                if (event.startTime < WORK_DAY_START_MINUTES) {
                    ...
                    event.endTime = Math.min(event.endTime + minOtherMinutes,
                            DAY_IN_MINUTES - 1);
            ...
            if (segments.size() == 0) {
                addNewSegment(segments, event, strands, firstJulianDay, 0,
                        minMinutes);
                continue;
            }
            ...
            int startMinute = (event.startDay - firstJulianDay)
                    * DAY_IN_MINUTES + event.startTime;
            int endMinute = Math.max((event.endDay - firstJulianDay)
                    * DAY_IN_MINUTES + event.endTime, startMinute + minMinutes);
            ...
            if (startMinute < lastSegment.endMinute) {
                ...
                for (; i >= 0
                        && startMinute <= (currSegment = segments.get(i)).endMinute; i--) {
                    ...
                    if (endMinute < currSegment.endMinute - minMinutes) {
                        ...
                        if (DEBUG) {
                            Log.d(TAG,
                                    "Added rhs, curr:" + currSegment.toString()
                                            + " i:"
                    ...
                    if (startMinute > currSegment.startMinute + minMinutes) {
                        ...
                        if (DEBUG) {
                            Log.d(TAG,
                                    "Added lhs, curr:" + currSegment.toString()
                                            + " i:"
                                            + segments.get(i).toString());
                        }
                    }
                    ...
                    if (i + 1 < segments.size()) {
                        ...
                        if (rhs.color == CONFLICT_COLOR
                            ...
                            rhs.startMinute = Math.min(currSegment.startMinute,
                                    rhs.startMinute);
                    ...
                    if (i - 1 >= 0) {
                        ...
                        if (lhs.color == CONFLICT_COLOR
                            ...
                            lhs.endMinute = Math.max(currSegment.endMinute,
                                    lhs.endMinute);
                            segments.remove(currSegment);
                            strands.get(currSegment.color).count--;
                            // point at the new current segment
            ...
            if (endMinute > lastSegment.endMinute) {
                addNewSegment(segments, event, strands, firstJulianDay,
                        lastSegment.endMinute, minMinutes);
            }
        }
    ...
    private static void addAllDayToStrands(Event event,
            HashMap<Integer, DNAStrand> strands, int firstJulianDay, int numDays) {
    ...
    private static void weaveDNAStrands(LinkedList<DNASegment> segments,
            int firstJulianDay, HashMap<Integer, DNAStrand> strands, int top,
            int bottom, int[] dayXs) {
        ...
        for (DNASegment segment : segments) {
            ...
            y0 = top
                    + getPixelOffsetFromMinutes(dayStartMinute, workDayHeight,
                            remainderHeight);
            y1 = top
                    + getPixelOffsetFromMinutes(dayEndMinute, workDayHeight,
                            remainderHeight);
            if (DEBUG) {
                Log.d(TAG, "Adding " + Integer.toHexString(segment.color)
                        + " at x,y0,y1: " + x + " " + y0 + " " + y1 + " for "
    ...
    private static int getPixelOffsetFromMinutes(int minute, int workDayHeight,
        ...
        if (minute < WORK_DAY_START_MINUTES) {
            ...
            y = remainderHeight + workDayHeight
                    + (minute - WORK_DAY_END_MINUTES) * remainderHeight
                    / WORK_DAY_END_LENGTH;
        }
        return y;
    }
     ...
     */
    private static void addNewSegment(LinkedList<DNASegment> segments,
            Event event, HashMap<Integer, DNAStrand> strands,
            int firstJulianDay, int minStart, int minMinutes) {
        if (event.startDay > event.endDay) {
        ...
        if (event.startDay != event.endDay) {
            ...
            while (lhs.startDay != event.endDay) {
                addNewSegment(segments, lhs, strands, firstJulianDay, minStart,
                        minMinutes);
     ...
     */
    private static DNAStrand getOrCreateStrand(
            HashMap<Integer, DNAStrand> strands, int color) {
    ...
    public static void setUpSearchView(SearchView view, Activity act) {
        SearchManager searchManager = (SearchManager) act
                .getSystemService(Context.SEARCH_SERVICE);
        view.setSearchableInfo(searchManager.getSearchableInfo(act
                .getComponentName()));
     ...
     * @return
     */
    public static int getWeekNumberFromTime(long millisSinceEpoch,
            Context context) {
        ...
        if (weekTime.weekDay == Time.SUNDAY
            ...
            weekTime.monthDay++;
            weekTime.normalize(true);
        } else if (weekTime.weekDay == Time.SATURDAY
    ...
    public static int getWeekNumberFromCalendar(long millisSinceEpoch,
            Context context) {
        Calendar calendar = Calendar.getInstance(TimeZone
                .getTimeZone(getTimeZone(context, null)));
        calendar.setTimeInMillis(millisSinceEpoch);
        calendar.setFirstDayOfWeek(getFirstDayOfWeekAsCalendar(context));
     ...
     * @return
     */
    public static String getDayOfWeekString(int julianDay, int todayJulianDay,
            long millis, Context context) {
        getTimeZone(context, null);
        int flags = DateUtils.FORMAT_SHOW_WEEKDAY;
        String dayViewText;
        /*
         * if (julianDay == todayJulianDay) { dayViewText =
         * context.getString(R.string.agenda_today,
         * mTZUtils.formatDateRange(context, millis, millis, flags).toString());
         * } else if (julianDay == todayJulianDay - 1) { dayViewText =
         * context.getString(R.string.agenda_yesterday,
         * mTZUtils.formatDateRange(context, millis, millis, flags).toString());
         * } else if (julianDay == todayJulianDay + 1) { dayViewText =
         * context.getString(R.string.agenda_tomorrow,
         * mTZUtils.formatDateRange(context, millis, millis, flags).toString());
         * } else { dayViewText = mTZUtils.formatDateRange(context, millis,
    ...
    public static String getDisplayedDatetime(long startMillis, long endMillis,
            long currentMillis, String localTimezone, boolean allDay,
            boolean lunarShow, Context context) {
        ...
        boolean singleDayEvent = false;
        if (allDay) {
            // All day events require special timezone adjustment.
            long localStartMillis = convertAlldayUtcToLocal(null, startMillis,
                    localTimezone);
            long localEndMillis = convertAlldayUtcToLocal(null, endMillis,
                    localTimezone);
            if (singleDayEvent(localStartMillis, localEndMillis,
                    currentTime.gmtoff)) {
            ...
            if (datetimeString == null) {
                ...
                if (lunarShow) {
                    // lunar date
                    datetimeString = formatLunarDateRange(context, startMillis,
                            endMillis, localTimezone, allDay, singleDayEvent,
                            flagsDate);
                } else {
                    Formatter f = new Formatter(new StringBuilder(50),
                            Locale.getDefault());
                    datetimeString = DateUtils.formatDateRange(context, f,
                            startMillis, endMillis, flagsDate,
            ...
            if (singleDayEvent(startMillis, endMillis, currentTime.gmtoff)) {
                // Format the time.
                String timeString = Utils.formatDateRange(context, startMillis,
                        endMillis, flagsTime);

                // If possible, use "Today" or "Tomorrow" instead of a full date
                // string.
                int todayOrTomorrow = isTodayOrTomorrow(context.getResources(),
                        startMillis, currentMillis, currentTime.gmtoff);
                singleDayEvent = true;
                if (TODAY == todayOrTomorrow) {
                    // Example: "Today at 1:00pm - 2:00 pm"
                    datetimeString = resources.getString(
                            R.string.today_at_time_fmt, timeString);
                } else if (TOMORROW == todayOrTomorrow) {
                    // Example: "Tomorrow at 1:00pm - 2:00 pm"
                    datetimeString = resources.getString(
                            R.string.tomorrow_at_time_fmt, timeString);
                } else {
                    String dateString = "";
                    if (lunarShow) {
                        dateString = formatLunarDateRange(context, startMillis,
                                endMillis, localTimezone, allDay,
                                singleDayEvent, flagsDate);
                    } else {
                        dateString = Utils.formatDateRange(context,
                                startMillis, endMillis, flagsDate);
                    }
                    // Format the full date. Example:
                    // "Thursday, April 12, 1:00pm - 2:00pm"
                    datetimeString = resources.getString(
                            R.string.date_time_fmt, dateString, timeString);
                }
            } else {
                int flagsDatetime = flagsDate | flagsTime
                ...
                if (lunarShow) {
                    ...
                    datetimeString = Utils.formatDateRange(context,
                            startMillis, endMillis, flagsDatetime);
                }
            }
        }
    ...
    public static String formatLunarDateRange(Context context,
            long startMillis, long endMillis, String tz, boolean allDay,
            final int flags) {
        boolean singleDayEvent = false;
        Time currentTime = new Time(tz);
        currentTime.setToNow();
        singleDayEvent = singleDayEvent(startMillis, endMillis,
                currentTime.gmtoff);
        return formatLunarDateRange(context, startMillis, endMillis, tz,
                allDay, singleDayEvent, flags);
    }

    public static String formatLunarDateRange(Context context,
            long startMillis, long endMillis, String tz, boolean allDay,
            boolean singleDayEvent, final int flags) {
        if (allDay) {
            tz = Time.TIMEZONE_UTC;
            endMillis = endMillis - DateUtils.DAY_IN_MILLIS;
        }
        Time t = new Time(tz);
        t.set(startMillis);
        StringBuilder out = new StringBuilder();
        LunarCalendar.LunarInfo startLunarInfo = LunarCalendar.solar2lunar(
                t.year, t.month + 1, t.monthDay);
        boolean showWeekDay = (flags & DateUtils.FORMAT_SHOW_WEEKDAY) != 0;
        boolean showTime = (flags & DateUtils.FORMAT_SHOW_TIME) != 0;
        int timeflags = (flags & DateUtils.FORMAT_SHOW_TIME)
                | (flags & DateUtils.FORMAT_24HOUR);
        if (startLunarInfo != null) {
            out.append(startLunarInfo.iMonthCn).append(startLunarInfo.iDayCn);
            if (showWeekDay) {
                if ((flags & DateUtils.FORMAT_ABBREV_WEEKDAY) != 0) {
                    out.append(startLunarInfo.ncWeek);
                } else {
                    out.append(startLunarInfo.ncWeek);
                }
            }
        ...
        if (showTime && !singleDayEvent) {
            String timeStr = Utils.formatDateRange(context, startMillis,
                    startMillis, timeflags);
            out.append(" ").append(timeStr);
        }
        if (!singleDayEvent) {
            t.set(endMillis);
            LunarCalendar.LunarInfo endLunarInfo = LunarCalendar.solar2lunar(
                    t.year, t.month + 1, t.monthDay);
            if (endLunarInfo != null) {
                if (endLunarInfo.lYear != startLunarInfo.lYear) {
                    String lunarYearStr = context
                            .getString(R.string.lunar_year);
                    out.insert(0, startLunarInfo.lYear + lunarYearStr);
                    out.append(" �?").append(endLunarInfo.lYear + lunarYearStr)
                            .append(endLunarInfo.iMonthCn);
                } else if (endLunarInfo.lMonth != startLunarInfo.lMonth) {
                    out.append(" - ").append(endLunarInfo.iMonthCn);
                } else {
                    out.append(" - ");
                }
                out.append(endLunarInfo.iDayCn);
                if (showWeekDay) {
                    if ((flags & DateUtils.FORMAT_ABBREV_WEEKDAY) != 0) {
                        out.append(startLunarInfo.ncWeek);// abbr
                    } else {
                        out.append(startLunarInfo.ncWeek);
                    }
                }
                if (showTime) {
                    String timeStr = Utils.formatDateRange(context, endMillis,
                            endMillis, timeflags);
                    out.append(" ").append(timeStr);
                }
            }
        } else {
            if (endMillis != startMillis && showTime) {
                String timeStr = Utils.formatDateRange(context, startMillis,
                        endMillis, timeflags);
                out.append(timeStr);
            }
     ...
     */
    public static String getDisplayedTimezone(long startMillis,
            String localTimezone, String eventTimezone) {
        ...
        if (!TextUtils.equals(localTimezone, eventTimezone)) {
            ...
            if (tz == null || tz.getID().equals("GMT")) {
                ...
                tzDisplay = tz.getDisplayName(startTime.isDst != 0,
                        TimeZone.SHORT);
            }
        }
        return tzDisplay;
     ...
     */
    private static boolean singleDayEvent(long startMillis, long endMillis,
            long localGmtOffset) {
     ...
     */
    public static Intent createEmailAttendeesIntent(Resources resources,
            String eventTitle, String body, List<String> toEmails,
            List<String> ccEmails, String ownerAccount) {
        ...
        if (toEmails.size() <= 0) {
            if (ccEmails.size() <= 0) {
                ...
                throw new IllegalArgumentException(
                        "Both toEmails and ccEmails are empty.");
            }
        ...
        String subject = null;
        if (eventTitle != null) {
            subject = resources.getString(R.string.email_subject_prefix)
                    + eventTitle;
        }
        ...
        /*
         * if (toList.size() > 1) { for (int i = 1; i < toList.size(); i++) { //
         * The Email app requires repeated parameter settings instead of // a
         * single comma-separated list. uriBuilder.appendQueryParameter("to",
         * toList.get(i)); } }
         */
        String emailDiv = ",";
        StringBuilder toListSBuilder = new StringBuilder();
        for (String email : toList) {
            toListSBuilder.append(email);
            toListSBuilder.append(emailDiv);
        }
        uriBuilder.appendQueryParameter("to", toListSBuilder.toString());
        ...
        if (ccList != null && ccList.size() > 0) {
            StringBuilder ccListSBuilder = new StringBuilder();
            for (String email : ccList) {
                ccListSBuilder.append(email);
                ccListSBuilder.append(emailDiv);
            }
            uriBuilder.appendQueryParameter("cc", ccListSBuilder.toString());
        }
        ...
        return Intent.createChooser(emailIntent,
                resources.getString(R.string.email_picker_label));
    }
     ...
     */
    public static void setTodayIcon(LayerDrawable icon, Context c,
            String timezone) {
        DayOfMonthDrawable today;

        // Reuse current drawable if possible
        Drawable currentDrawable = icon
                .findDrawableByLayerId(R.id.today_icon_day);
        if (currentDrawable != null
                && currentDrawable instanceof DayOfMonthDrawable) {
        ...
        icon.setTint(c.getResources().getColor(
                R.color.cale_actionbar_icon_color));
    }
    ...
    public static BroadcastReceiver setTimeChangesReceiver(Context c,
            Runnable callback) {
    ...
    public static String[] getQuickResponses(Context context) {
        ...
        if (s == null) {
            s = context.getResources().getStringArray(
                    R.array.quick_response_defaults);
        }
    ...
    public static Spannable extendedLinkify(String text, boolean lastDitchGeo) {
        ...
        if (!defaultPhoneRegion.equals("US")) {
            ...
            URLSpan[] spans = spanText.getSpans(0, spanText.length(),
                    URLSpan.class);
            if (spans.length == 1) {
         ...
         */
        boolean linkifyFoundLinks = Linkify.addLinks(spanText, Linkify.ALL
         ...
         */
        if (lastDitchGeo && !text.isEmpty() && !linkifyFoundLinks
                && phoneCount == 0 && coordCount == 0) {
    ...
    private static int findNanpMatchEnd(CharSequence text, int startPos) {
        ...
        if (text.length() > startPos + 4
                && text.subSequence(startPos, startPos + 4).toString()
    ...
    public static ArrayList<ReminderEntry> readRemindersFromBundle(Bundle bundle) {
        ...
        ArrayList<Integer> reminderMinutes = bundle
                .getIntegerArrayList(EventInfoFragment.BUNDLE_KEY_REMINDER_MINUTES);
        ArrayList<Integer> reminderMethods = bundle
                .getIntegerArrayList(EventInfoFragment.BUNDLE_KEY_REMINDER_METHODS);
        if (reminderMinutes == null || reminderMethods == null) {
            if (reminderMinutes != null || reminderMethods != null) {
                String nullList = (reminderMinutes == null ? "reminderMinutes"
                        : "reminderMethods");
                Log.d(TAG, String.format(
                        "Error resolving reminders: %s was null", nullList));
            }
            return null;
        }
    ...
    private static Boolean sAbroadBranch = null;
    ...
    public static boolean isAbroadBranch(Context cx) {
        if (sAbroadBranch == null) {
            String val = SystemPropertiesProxy.get(cx, "ro.gios.custom");
            sAbroadBranch = val.equalsIgnoreCase("abroad");
        }
        return sAbroadBranch;
    }

    public static final int REQUEST_PERMISSIONS_NULL = 0; // for just check if
                                                          // has permission.
    public static final int REQUEST_PERMISSIONS_CALENDAR = 1;
    public static final int REQUEST_PERMISSIONS_CONTACTS_ADD = 2;
    public static final int REQUEST_PERMISSIONS_CONTACTS_EDIT = 3;
    public static final int REQUEST_PERMISSIONS_CALLLOG = 4;
    public static final int REQUEST_PERMISSIONS_EXTERNAL_STORAGE = 5;

    @SuppressLint("NewApi")
    @TargetApi(23)
    public static boolean checkAndRequestPermission(Activity activity,
            String sPermission, int requestCode) {
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP_MR1) {
            return true;
        }
        if (activity.checkSelfPermission(sPermission) != PackageManager.PERMISSION_GRANTED) {
            // without permission
            if (requestCode == REQUEST_PERMISSIONS_NULL) {
                return false;
            }
            // if(!activity.shouldShowRequestPermissionRationale(sPermission)){
            // if (DEBUG) {
            // Log.d(TAG, "1.permission" + sPermission +
            // ":have checked 'Don't ask again'");
            // }
            // Intent intent = new
            // Intent("android.intent.action.APP_PERMISSION_DIALOG");
            // intent.addCategory(Intent.CATEGORY_DEFAULT);
            // intent.putExtra("requested-package", activity.getPackageName());
            // intent.putExtra("requested-permission", new String[] {
            // sPermission });
            // activity.startActivity(intent);
            // }
            activity.requestPermissions(new String[] { sPermission },
                    requestCode);
            return false;
        }
        // with permission
    ...
    public static boolean checkAndRequestPermission(Fragment frg,
            String sPermission, int requestCode) {
        if (Build.VERSION.SDK_INT <= Build.VERSION_CODES.LOLLIPOP_MR1) {
            return true;
        }
        if (frg.getContext().checkSelfPermission(sPermission) != PackageManager.PERMISSION_GRANTED) {
            // without permission
            if (requestCode == REQUEST_PERMISSIONS_NULL) {
                return false;
            }
            // if(!frg.shouldShowRequestPermissionRationale(sPermission)){
            // if (DEBUG) {
            // Log.d(TAG, "1.permission" + sPermission +
            // ":have checked 'Don't ask again'");
            // }
            ...
            // Intent("android.intent.action.APP_PERMISSION_DIALOG");
            // intent.addCategory(Intent.CATEGORY_DEFAULT);
            // intent.putExtra("requested-package",
            // frg.getContext().getPackageName());
            // intent.putExtra("requested-permission", new String[] {
            // sPermission });
            // frg.startActivity(intent);
            // }
            frg.requestPermissions(new String[] { sPermission }, requestCode);
            return false;
        }
        // with permission
        return true;
    ...
    public static void showMessage(Context context, String message,
        ...
        showMessage(context, message, null, false, okListener);
    }

    public static void showMessage(Context context, String message,
            String title, boolean hasCancelBtn,
            DialogInterface.OnClickListener okListener) {
    ...
    public static boolean compareWithCurLang(Context context,
            String compareLangCode) {
        Locale locale = context.getResources().getConfiguration().locale;
        String curLang = locale.toString();
        if (curLang.startsWith(compareLangCode)) {
            return true;
        }
        return false;
    }

    private static double sMFVVersion = 0.0;

    public static double getBuildMFVVersion(Context cx) {
        if (sMFVVersion == 0.0) {
            String val = SystemPropertiesProxy.get(cx, "ro.build.GiOs_version");
            if (!TextUtils.isEmpty(val)) {
                sMFVVersion = Double.parseDouble(val);
            }
        }
        return sMFVVersion;
    }

    public static int convertDp2Px(Context context, int old) {
        return (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,
                old, context.getResources().getDisplayMetrics());
    }

    public static float convertDp2Px(Context context, float old) {
    ...
    public static int getNormalNavigationBarHeight(final Context ctx) {
        try {
            ...
            int rid = res.getIdentifier("config_showNavigationBar", "bool",
                    "android");
            if (rid > 0) {
            ...
            LogUtils.d("getNormalNavigationBarHeight() exception:"
                    + e.getMessage());
        }
        return 0;
    }
