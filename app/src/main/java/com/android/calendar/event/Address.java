...
public class Address {
    ...
    public static final int VALID_ADDRESS = 1;
    public static final int INVALIDRFC822_ADDRESS = 2;
    public static final int NOCERTIFICATE_ADDRESS = 4;
    public static final int OVERPERIODCERTIFICATE_ADDRESS = 8;
    public static final int NORELATECERTIFICATE_ADDRESS = 16;
    public static final int NO_BIND_CERTIFICATE_ADDRESS = 32;
    ...
    private static final Pattern REMOVE_OPTIONAL_BRACKET = Pattern
            .compile("^<?([^>]+)>?$");
    // Regex that matches personal name surrounded by '""' optionally.
    // '^"?([^"]+)"?$'
    private static final Pattern REMOVE_OPTIONAL_DQUOTE = Pattern
            .compile("^\"?([^\"]*)\"?$");
     ...
     * 
     * @param personal
     *            name part of email address as UTF-16 string. Null is
     *            acceptable.
     */
    public void setPersonal(String personal) {
        if (personal != null) {
            personal = REMOVE_OPTIONAL_DQUOTE.matcher(personal)
                    .replaceAll("$1");
    ...
    public static Address[] unpack(String addressList) {
        ...
        while (pairStartIndex < length) {
            pairEndIndex = addressList.indexOf(LIST_DELIMITER_EMAIL,
                    pairStartIndex);
            if (pairEndIndex == -1) {
                pairEndIndex = length;
            }
            ...
            if (addressEndIndex == -1 || pairEndIndex <= addressEndIndex) {
                ...
                address = new Address(addressList.substring(pairStartIndex,
                        addressEndIndex), addressList.substring(
                        addressEndIndex + 1, pairEndIndex));
                // only update addressEndIndex when we use the
                // LIST_DELIMITER_PERSONAL
                addressEndIndex = addressList.indexOf(LIST_DELIMITER_PERSONAL,
                        pairEndIndex + 1);
            }
    ...
    public static Address[] legacyUnpack(String addressList) {
        ...
        while (pairStartIndex < length) {
            ...
            if (addressEndIndex == -1 || addressEndIndex > pairEndIndex) {
                address =
                /* Utility. */fastUrlDecode(addressList.substring(
                        pairStartIndex, pairEndIndex));
            } else {
                address =
                /* Utility. */fastUrlDecode(addressList.substring(
                        pairStartIndex, addressEndIndex));
                personal =
                /* Utility. */fastUrlDecode(addressList.substring(
                        addressEndIndex + 1, pairEndIndex));
            }
