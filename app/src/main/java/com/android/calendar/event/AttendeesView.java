...
public class AttendeesView extends LinearLayout implements View.OnClickListener {
    ...
    public AttendeesView(Context context, AttributeSet attrs) {
        ...
        mPresenceQueryHandler = new PresenceQueryHandler(
                context.getContentResolver());

        final Resources resources = context.getResources();
        mDefaultBadge = resources.getDrawable(R.drawable.ic_contact_picture);
        mNoResponsePhotoAlpha = resources
                .getInteger(R.integer.noresponse_attendee_photo_alpha_level);
        mDefaultPhotoAlpha = resources
                .getInteger(R.integer.default_attendee_photo_alpha_level);

        // Create dividers between groups of attendees (accepted, declined,
        // etc...)
    ...
    private View constructDividerView(CharSequence label) {
        final TextView textView = (TextView) mInflater.inflate(
                R.layout.event_info_label, this, false);
        textView.setText(label);
        textView.setClickable(false);
        return textView;
    }

    // Add the number of attendees in the specific status (corresponding to the
    // divider) in
    // parenthesis next to the label
    private void updateDividerViewLabel(View divider, CharSequence label,
            int count) {
        if (count <= 0) {
    ...
    /**
     * Inflates a layout for a given attendee view and set up each element in
     * it, and returns the constructed View object. The object is also stored in
     * {@link AttendeeItem#mView}.
     */
    ...
    /**
     * Set up each element in {@link AttendeeItem#mView} using the latest
     * information. View object is reused.
     */
    private View updateAttendeeView(AttendeeItem item) {
        ...
        nameView.setText(TextUtils.isEmpty(attendee.mName) ? attendee.mEmail
                : attendee.mName);
        if (item.mRemoved) {
            nameView.setPaintFlags(Paint.STRIKE_THRU_TEXT_FLAG
                    | nameView.getPaintFlags());
        } else {
            nameView.setPaintFlags((~Paint.STRIKE_THRU_TEXT_FLAG)
                    & nameView.getPaintFlags());
        }
        ...
        if (item.mRemoved) {
            Drawable add = resources.getDrawable(R.drawable.add_circle);
            add.setTint(resources.getColor(R.color.cale_icon_color));
            button.setImageDrawable(add);
            button.setContentDescription(resources
                    .getString(R.string.accessibility_add_attendee));
        } else {
            Drawable remove = resources.getDrawable(R.drawable.remove_circle);
            remove.setTint(resources.getColor(R.color.cale_icon_color));
            button.setImageDrawable(remove);
            button.setContentDescription(resources
                    .getString(R.string.accessibility_remove_attendee));
        }
        button.setOnClickListener(this);

        final QuickContactBadge badgeView = (QuickContactBadge) view
        ...
        // If we know the lookup-uri of the contact, it is a good idea to set
        // this here. This
        // allows QuickContact to be started without an extra database lookup.
        // If we don't know
        // the lookup uri (yet), we can set Email and QuickContact will lookup
        // once tapped.
    ...
    public boolean contains(Attendee attendee) {
        ...
        for (int i = 0; i < size; i++) {
            ...
            if (TextUtils
                    .equals(attendee.mEmail, attendeeItem.mAttendee.mEmail)) {
                return true;
            }
        }
        return false;
    ...
    public void clearAttendees() {
        ...
        // Before clearing the views, save all the badges. The updateAtendeeView
        // will use the saved
        // photo instead of the default badge thus prevent switching between the
        // two while the
        ...
        for (int i = 0; i < size; i++) {
            ...
            AttendeeItem attendeeItem = (AttendeeItem) view.getTag();
            mRecycledPhotos.put(attendeeItem.mAttendee.mEmail,
                    attendeeItem.mBadge);
        }
    ...
    private void addOneAttendee(Attendee attendee) {
        ...
        default: {
            final int startIndex = (mYes == 0 ? 0 : 1 + mYes)
                    + (mNo == 0 ? 0 : 1 + mNo) + (mMaybe == 0 ? 0 : 1 + mMaybe);
            updateDividerViewLabel(mDividerForNoResponse, mEntries[0],
                    mNoResponse + 1);
            if (mNoResponse == 0) {
        ...
        if (attendee.mIdentity != null && attendee.mIdNamespace != null) {
            ...
            selection = Data.MIMETYPE + "=? AND " + Identity.IDENTITY
                    + "=? AND " + Identity.NAMESPACE + "=?";
            selectionArgs = new String[] { Identity.CONTENT_ITEM_TYPE,
                    attendee.mIdentity, attendee.mIdNamespace };
        } else {
            // Query by email
            uri = Uri.withAppendedPath(Email.CONTENT_LOOKUP_URI,
                    Uri.encode(attendee.mEmail));
        }

        mPresenceQueryHandler.startQuery(item.mUpdateCounts + 1, item, uri,
    ...
    public void addAttendees(String attendees) {
        final LinkedHashSet<Rfc822Token> addresses = EditEventHelper
                .getAddressesFromList(attendees, mValidator);
        synchronized (this) {
            for (final Rfc822Token address : addresses) {
                final Attendee attendee = new Attendee(address.getName(),
                        address.getAddress());
    ...
    /**
     * Returns true when the attendee at that index is marked as "removed" (the
     * name of the attendee is shown with a strike through line).
     */
    ...
    private class PresenceQueryHandler extends AsyncQueryHandler {
        ...
        @Override
        protected void onQueryComplete(int queryIndex, Object cookie,
                Cursor cursor) {
                ...
                if (item.mUpdateCounts < queryIndex) {
                    ...
                    if (cursor.moveToFirst()) {
                        final long contactId = cursor
                                .getLong(EMAIL_PROJECTION_CONTACT_ID_INDEX);
                        final Uri contactUri = ContentUris.withAppendedId(
                                Contacts.CONTENT_URI, contactId);

                        final String lookupKey = cursor
                                .getString(EMAIL_PROJECTION_CONTACT_LOOKUP_INDEX);
                        item.mContactLookupUri = Contacts.getLookupUri(
                                contactId, lookupKey);

                        final long photoId = cursor
                                .getLong(EMAIL_PROJECTION_PHOTO_ID_INDEX);
                        ...
                        if (photoId > 0) {
                            ...
                            // call update view to make sure that the lookup key
                            // gets set in
                            // the QuickContactBadge
                            updateAttendeeView(item);
                        }
                    } else {
                        // Contact not found. For real emails, keep the
                        // QuickContactBadge with
                        // its Email address set, so that the user can create a
                        // contact by tapping.
