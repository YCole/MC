...
/**
 * A cache for event colors and event color keys stored based upon calendar
...
public class EventColorCache implements Serializable {
     ...
     */
    public void insertColor(String accountName, String accountType,
            int displayColor, int colorKey) {
        mColorKeyMap.put(createKey(accountName, accountType, displayColor),
                colorKey);
    ...
    public int[] getColorArray(String accountName, String accountType) {
        ArrayList<Integer> colors = mColorPaletteMap.get(createKey(accountName,
                accountType));
        if (colors == null) {
            return null;
        }
    ...
    /**
     * Retrieve an event color's unique key based on account name, type, and
     * color.
     */
    public int getColorKey(String accountName, String accountType,
            int displayColor) {
        return mColorKeyMap.get(createKey(accountName, accountType,
                displayColor));
    }
    ...
    private String createKey(String accountName, String accountType) {
        return new StringBuilder().append(accountName).append(SEPARATOR)
    ...
    private String createKey(String accountName, String accountType,
        ...
        return new StringBuilder(createKey(accountName, accountType))
                .append(SEPARATOR).append(displayColor).toString();
    }
}
