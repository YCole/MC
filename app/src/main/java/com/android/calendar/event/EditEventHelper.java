...
public class EditEventHelper {
    ...
    /**
     * when event type is {@link #EventItem.VIEW_EVENT_LOCATION} action_data is location alert
     * radius and action_data1 is mapParam ,is json object string ,{}
     * 
     */
    public static final String EVENT_EXT_EVENT_TYPE = "eventType";
    public static final String EVENT_EXT_ACTION_DATA = "action_data";
    public static final String EVENT_EXT_ACTION_DATA1 = "action_data1";
    public static final String EVENT_EXT_ACTION_DATA2 = "action_data2";
    ...
    public static final String[] EVENT_PROJECTION = new String[] { Events._ID, // 0
            ...
            EVENT_EXT_EVENT_TYPE,// 25
            EVENT_EXT_ACTION_DATA,// 26
            EVENT_EXT_ACTION_DATA1,// 27
            EVENT_EXT_ACTION_DATA2,// 28
            Events.RDATE,// 29
    };
    ...
    protected static final int EVENT_INDEX_EVENT_TYPE_KEY = 25;
    protected static final int EVENT_INDEX_EVENT_ACTION_DATA_KEY = 26;
    protected static final int EVENT_INDEX_EVENT_ACTION_DATA1_KEY = 27;
    protected static final int EVENT_INDEX_EVENT_ACTION_DATA2_KEY = 28;
    protected static final int EVENT_INDEX_RDATE = 29;
    ...
    // Visible for testing
    static final String ATTENDEES_DELETE_PREFIX = Attendees.EVENT_ID
            + "=? AND " + Attendees.ATTENDEE_EMAIL + " IN (";
    ...
    static final String[] COLORS_PROJECTION = new String[] { Colors._ID, // 0
            Colors.ACCOUNT_NAME, Colors.ACCOUNT_TYPE, Colors.COLOR, // 1
            Colors.COLOR_KEY // 2
    };
    ...
    static final String[] ATTENDEES_PROJECTION = new String[] { Attendees._ID, // 0
    ...
    static final String ATTENDEES_WHERE = Attendees.EVENT_ID
            + "=? AND attendeeEmail IS NOT NULL";

    private String mMapParam = null;
    private int mLocationRaduis = 1000;
    ...
    public EditEventHelper(Context context) {
        // mService = ((AbstractCalendarActivity)
        // context).getAsyncQueryService();
     ...
     * 
     * @param model
     *            The event model to save
     * @param originalModel
     *            A model of the original event if it exists
     * @param modifyWhich
    ...
    public boolean saveEvent(CalendarEventModel model,
        ...
        if (DEBUG) {
            LogUtils.e("lxg Saving event model: " + model);
        }

        if (!mEventOk) {
            if (DEBUG) {
                LogUtils.e("Event no longer exists. Event was not saved.");
            }
            return false;
        }
        ...
        if (model == null) {
            LogUtils.e("Attempted to save null model.");
            return false;
        }
        if (!model.isValid()) {
            LogUtils.e("Attempted to save invalid model.");
            return false;
        }
        if (originalModel != null && !isSameEvent(model, originalModel)) {
            LogUtils.e("Attempted to update existing event but models didn't refer to the same "
                    + "event.");
            return false;
        }
        ...
        if (model.mUri != null && originalModel == null) {
            LogUtils.e("lxg Existing event but no originalModel provided. Aborting save.");
            return false;
        }
        Uri uri = null;
        if (model.mUri != null) {
        ...
        if (uri == null) {
            ...
            ContentProviderOperation.Builder b = ContentProviderOperation
                    .newInsert(Events.CONTENT_URI).withValues(values);
            ops.add(b.build());
            forceSaveReminders = true;

        } else if (TextUtils.isEmpty(model.mRrule)
                && TextUtils.isEmpty(originalModel.mRrule)
                && TextUtils.isEmpty(model.mRdate)
                && TextUtils.isEmpty(originalModel.mRdate)) {
            ...
            if (TextUtils.isEmpty(model.mRrule)) {
                ...
                if (isFirstEventInSeries(model, originalModel)) {
                    checkTimeDependentFields(originalModel, model, values,
                            modifyWhich);
                    ContentProviderOperation.Builder b = ContentProviderOperation
                            .newUpdate(uri).withValues(values);
                    ops.add(b.build());
                } else {
                    // We need to update the existing recurrence to end before
                    // the exception
                    // event starts. If the recurrence rule has a COUNT, we need
                    // to adjust
                    // that in the original and in the exception. This call
                    // rewrites the
                    // original event's recurrence rule (in "ops"), and returns
                    // a new rule
                    // for the exception. If the exception explicitly set a new
                    // rule, however,
        ...
        // add by hct.gengbin for location remind, begin
        if (model.mEventType == EventItem.VIEW_EVENT_LOCATION
                || (originalModel != null && originalModel.mEventType == EventItem.VIEW_EVENT_LOCATION)) {
        ...
        // add by hct.gengbin for location remind, end
        ...
        if (hasAttendeeData && model.mOwnerAttendeeId == -1) {
            ...
            if (model.mAttendeesList.size() != 0
                    && Utils.isValidEmail(ownerEmail)) {
                ...
                if (newEvent) {
                    b = ContentProviderOperation.newInsert(
                            Attendees.CONTENT_URI).withValues(values);
            ...
            if (DEBUG) {
                LogUtils.e("Setting attendee status to "
                        + model.mSelfAttendeeStatus);
            }
            Uri attUri = ContentUris.withAppendedId(Attendees.CONTENT_URI,
                    model.mOwnerAttendeeId);
        ...
        if (hasAttendeeData && (newEvent || uri != null)) {
            ...
            // has changed it
            if (newEvent
                    || !TextUtils.equals(originalAttendeesString, attendees)) {
                ...
                if (!newEvent) {
                    ...
                    if (removedAttendees.size() > 0) {
                        b = ContentProviderOperation
                                .newDelete(Attendees.CONTENT_URI);
                ...
                if (newAttendees.size() > 0) {
                    ...
                    for (Attendee attendee : newAttendees.values()) {
                        ...
                        if (newEvent) {
                            b = ContentProviderOperation.newInsert(
                                    Attendees.CONTENT_URI).withValues(values);
                            b.withValueBackReference(Attendees.EVENT_ID,
                                    eventIdIndex);
                        } else {
                        ...
                        boolean bSmartReminder = Utils.getConfigBool(mContext,
                                R.bool.smart_reminder);
                        if (bSmartReminder
                                && attendee.mEmail.indexOf("@") == -1) {// &&
                                                                        // isValidEmail(attendee.mEmail)){
                            LogUtils.e("saveEvent, attendee.mEmail="
                                    + attendee.mEmail);

                            String message = FormatMessage(model);
                            LogUtils.e("saveEvent:" + message);

                            SendSMS(attendee.mEmail, message);
                        }
                    }
                }
            }
    ...
    private static void SendSMS(String address, String content) {
        SmsManager smsManager = SmsManager.getDefault();

        LogUtils.e("SendSMS, address = " + address);
        LogUtils.e("SendSMS, content = " + content);

        /*
         * List<String> divideContents = smsManager.divideMessage(content);
         * 
         * for(String str : divideContents ) {
         * LogUtils.e("SendSMS, str = "+str);
         * smsManager.sendTextMessage(address, null, str, null, null); }
         */

        smsManager.sendMultipartTextMessage(address, null,
                smsManager.divideMessage(content), null, null);
    }

    private static String FormatMessage(CalendarEventModel model) {
        SimpleDateFormat df = new SimpleDateFormat("yyyy/MM/dd HH:mm");
        SimpleDateFormat df1 = new SimpleDateFormat("yyyy/MM/dd");
        Date dtstart = new Date();
        Date dtend = new Date();
        dtstart.setTime(model.mStart);
        dtend.setTime(model.mEnd);

        String title = model.mTitle == null ? "" : model.mTitle;
        String location = model.mLocation == null ? "" : model.mLocation;

        String message = "\u4e3b\u9898:" + title + "\n\u5730\u70b9:" + location
                + "\n";

        if (model.mAllDay) {
            dtend.setDate(dtend.getDate() - 1);
            String StrStart = df1.format(dtstart);
            String StrEnd = df1.format(dtend);
            if (StrStart.equals(StrEnd)) {
                message = message + StrStart + "\n\u63d0\u9192:";
            } else {
                message = message + StrStart + "-" + StrEnd + "\n\u63d0\u9192:";
            }
        } else {
            message = message + df.format(dtstart) + "-" + df.format(dtend)
                    + "\n\u63d0\u9192:";
        }

        if (model.mReminders.size() > 0) {
            for (int i = 0; i < model.mReminders.size(); i++) {
                int min = model.mReminders.get(i).getMinutes();
                if (min >= 1440) {
                    message = message + "\u63d0\u524d" + min / 1440 + "\u5929;";
                } else if (min >= 60) {
                    message = message + "\u63d0\u524d" + min / 60
                            + "\u5c0f\u65f6;";
                } else if (min > 0) {
                    message = message + "\u63d0\u524d" + min + "\u5206\u949f;";
                } else {
                    message = message + "\u51c6\u65f6;";
                }
            }
        ...
        LogUtils.e("model.mRrule = " + model.mRrule);

        String format_weekly = "(?<=FREQ=WEEKLY;WKST=(SU|SA|MO);BYDAY=)(.*)";
        Pattern ptn_weekly = Pattern.compile(format_weekly);

        // String format_monthweek =
        // "(?<=\u6bcf\u6708\u7b2c)(.*?)\u4e2a\u5468(.*?)(?=\u3002)";
        String format_monthweek = "(?<=FREQ=MONTHLY;WKST=(SU|SA|MO);BYDAY=)(.*)";
        Pattern ptn_monthweek = Pattern.compile(format_monthweek);

        // String format_monthly = "(?<=\u6bcf\u6708)(.*?)(?=\u65e5)";
        String format_monthly = "(?<=FREQ=MONTHLY;WKST=(SU|SA|MO);BYMONTHDAY=)(.*)";
        Pattern ptn_monthly = Pattern.compile(format_monthly);

        if (model.mRrule == null) {
            message = message + "\u4e00\u6b21\u6027\u3002";
        } else {
            Matcher matcher_weekly = ptn_weekly.matcher(model.mRrule);
            Matcher matcher_monthweek = ptn_monthweek.matcher(model.mRrule);
            Matcher matcher_monthly = ptn_monthly.matcher(model.mRrule);

            if (model.mRrule.indexOf("FREQ=DAILY;") != -1) {
                message = message + "\u6bcf\u5929\u3002";
            } else if (model.mRrule.indexOf("FREQ=WEEKLY;") != -1
                    && model.mRrule.indexOf("BYDAY=MO,TU,WE,TH,FR") != -1) {
                message = message + "\u5de5\u4f5c\u65e5\u3002";
            } else if (model.mRrule.indexOf("FREQ=YEARLY;") != -1) {
                message = message + "\u6bcf\u5e74\u3002";
            } else if (matcher_weekly.find()) {
                LogUtils.e("matcher_weekly FIND");
                String weeknum = matcher_weekly.group(0);
                LogUtils.e("weeknum = " + weeknum);
                String replace_tab[][] = { { "\u65e5", "SU" },
                        { "\u516d", "SA" }, { "\u4e94", "FR" },
                        { "\u56db", "TH" }, { "\u4e09", "WE" },
                        { "\u4e8c", "TU" }, { "\u4e00", "MO" } };
                for (int i = 0; i < replace_tab.length; i++) {
                    weeknum = weeknum.replaceAll(replace_tab[i][1],
                            replace_tab[i][0]);
                }
                message = message + "\u6bcf\u5468" + weeknum + "\u3002";
                ...
                for (int i = 0; i < replace_tab.length; i++) {
                    monthweek = monthweek.replaceAll(replace_tab[i][1],
                            replace_tab[i][0]);
                    LogUtils.e("GetFormatedMsg, i = " + i
                            + ", weeknum replaced = " + monthweek);
                }
                message = message + "\u6bcf\u6708\u7b2c" + monthweek + "\u3002";
            } else if (matcher_monthly.find()) {
                String monthdaynum = matcher_monthly.group(0);
                message = message + "\u6bcf\u6708" + monthdaynum
                        + "\u65e5\u3002";
            }
        }
        message = message
    ...
    public static LinkedHashSet<Rfc822Token> getAddressesFromList(String list,
        ...
        while (addressIterator.hasNext()) {
            ...
            if (!validator.isValid(address.getAddress())) {
                Log.v(TAG, "Dropping invalid attendee email address: "
                        + address.getAddress());
    ...
    void checkTimeDependentFields(CalendarEventModel originalModel,
        ...
        String oldRdate = originalModel.mRdate;
        ...
        if ((TextUtils.isEmpty(oldRrule) || TextUtils.isEmpty(newRrule))
                && (TextUtils.isEmpty(oldRdate) || TextUtils.isEmpty(newRdate))) {
            return;
        }
     ...
     * <p>
     * If the event's recurrence rule has a COUNT, we also need to reduce the
     * count in the RRULE for the exception event.
     * 
     * @param ops
     *            The list of operations to add the update to
     * @param originalModel
     *            The original event that we're updating
     * @param endTimeMillis
     *            The time before which the event must end (i.e. the start time
     *            of the exception event instance).
    ...
    public String updatePastEvents(ArrayList<ContentProviderOperation> ops,
        ...
        if (origRecurrence.count > 0) {
            /*
             * Generate the full set of instances for this recurrence, from the
             * first to the one just before endTimeMillis. The list should never
             * be empty, because this method should not be called for the first
             * instance. All we're really interested in is the *number* of
             * instances found.
             * 
             * TODO: the model assumes RRULE and ignores RDATE, EXRULE, and
             * EXDATE. For the current environment this is reasonable, but that
             * may not hold in the future.
             * 
             * TODO: if COUNT is 1, should we convert the event to
             * non-recurring? e.g. we do an "edit this and all future events" on
             * the 2nd instances.
             */
            RecurrenceSet recurSet = new RecurrenceSet(originalModel.mRrule,
                    null, null, null);
            RecurrenceProcessor recurProc = new RecurrenceProcessor();
            long[] recurrences;
            try {
                recurrences = recurProc.expand(dtstart, recurSet,
            ...
            if (recurrences.length == 0) {
                throw new RuntimeException(
                        "can't use this method on first instance");
            }

            EventRecurrence excepRecurrence = new EventRecurrence();
            excepRecurrence.parse(origRrule); // TODO: add+use a copy
                                              // constructor instead
            ...
            // to display it properly. For all-day events, the "until" time
            // string
            ...
            // "until" time.
            untilTime.set(endTimeMillis - 1000); // subtract one second (1000
                                                 // millis)
            if (origAllDay) {
                untilTime.hour = 0;
                untilTime.minute = 0;
                ...
                // This should no longer be necessary -- DTSTART should already
                // be in the correct
        ...
        ContentProviderOperation.Builder b = ContentProviderOperation
                .newUpdate(Uri.parse(originalModel.mUri)).withValues(
                        updateValues);
        ops.add(b.build());
     ...
     * @return
     */
    public static boolean isSameEvent(CalendarEventModel model,
     ...
     * 
     * @param ops
     *            the array of ContentProviderOperations
     * @param eventId
     *            the id of the event whose reminders are being updated
     * @param reminders
     *            the array of reminders set by the user
     * @param originalReminders
     *            the original array of reminders
     * @param forceSave
     *            if true, then save the reminders even if they didn't change
     * @return true if operations to update the database were added
     */
    public static boolean saveReminders(
            ArrayList<ContentProviderOperation> ops, long eventId,
    ...
    void addRecurrenceRule(ContentValues values, CalendarEventModel model) {
        ...
        addRecurrenceDuration(values, model);
        values.put(Events.DTEND, (Long) null);
    }

    void addRecurrenceDate(ContentValues values, CalendarEventModel model) {
        // If no good duration info exists assume the default
        addRecurrenceDuration(values, model);
        values.put(Events.DTEND, (Long) null);
    }

    void addRecurrenceDuration(ContentValues values, CalendarEventModel model) {
        long end = model.mEnd;
    ...
    static void updateRecurrenceRuleAndRdate(Context context, int selection,
            CalendarEventModel model, int weekStart) {
        if (selection == REPEATS_LUNAR) {

            boolean isAllDay = model.mAllDay;

            Time startTime = new Time(model.mTimezone);
            startTime.set(model.mStart);
            LunarCalendar.LunarDate info = LunarCalendar.solar2lunarDate(
                    startTime.year, startTime.month + 1, startTime.monthDay);
            if (info == null) {
                return;
            }
            Time startLunar = new Time(model.mTimezone);
            startLunar.set(info.lDay, info.lMonth, info.lYear);

            startTime.switchTimezone(Time.TIMEZONE_UTC);
            StringBuilder sb = new StringBuilder(formatRdate(startTime,
                    isAllDay)).append(",");
            startTime.switchTimezone(model.mTimezone);

            final int yearStart = info.year + 1;
            final int month = info.month;
            final int monthDay = info.day;

            for (int sYear = yearStart; sYear <= Utils.YEAR_MAX; sYear++) {
                int monthDayNum = LunarCalendar.monthDays(sYear, month);
                int realMonthDay = monthDay;
                if (monthDay > monthDayNum) {
                    realMonthDay = monthDayNum;
                }
                info = LunarCalendar.lunar2solarDate(sYear, month,
                        realMonthDay, false);
                if (info == null) {
                    LogUtils.e("updateRecurrenceRuleAndRdate lunar2solarDate is null sYear#"
                            + sYear
                            + ",month#"
                            + month
                            + ",monthDay#"
                ...
                startLunar.switchTimezone(model.mTimezone);
                startLunar.set(startTime.second, startTime.minute,
                        startTime.hour, info.lDay, info.lMonth - 1, info.lYear);
                startLunar.switchTimezone(Time.TIMEZONE_UTC);
                sb.append(formatRdate(startLunar, isAllDay)).append(",");
            }
            model.mRdate = sb.toString();
        } else {
            model.mRdate = null;
            updateRecurrenceRule(selection, model, weekStart);
        }
    }

    private static String formatRdate(Time t, boolean allday) {
        if (allday) {
            return String.format("%04d%02d%02d", t.year, t.month + 1,
                    t.monthDay);
        }
        return String.format("%04d%02d%02dT%02d%02d%02d", t.year, t.month + 1,
     ...
     *            The event to update
     * @param weekStart
     *            the week start day, specified as java.util.Calendar constants
     */
    static void updateRecurrenceRule(int selection, CalendarEventModel model,
        ...
        if (selection == DOES_NOT_REPEAT) {
            ...
            eventRecurrence.frequence = 365;
        }

        frequence = eventRecurrence.frequence;
     ...
     * @param cursor
     *            An event cursor that used {@link #EVENT_PROJECTION} for the
     *            query
     */
    public static void setModelFromCursor(CalendarEventModel model,
            Cursor cursor) {
        ...
        model.mRrule = rRule;
        model.mRdate = cursor.getString(EVENT_INDEX_RDATE);
        ...
        model.mIsOrganizer = model.mOwnerAccount
                .equalsIgnoreCase(model.mOrganizer);
        ...
        model.mEventType = cursor.getInt(EVENT_INDEX_EVENT_TYPE_KEY);
        model.mActionData = cursor.getString(EVENT_INDEX_EVENT_ACTION_DATA_KEY);
        model.mActionData1 = cursor
                .getString(EVENT_INDEX_EVENT_ACTION_DATA1_KEY);
        model.mModelUpdatedWithEventCursor = true;
    }

    /**
     * Uses a calendar cursor to fill in the given model This method assumes the
     * cursor used {@link #CALENDARS_PROJECTION} as it's query projection. It
     * uses the cursor to fill in the given model with all the information
     * available.
     * 
     * @param model
     *            The model to fill in
     * @param cursor
     *            An event cursor that used {@link #CALENDARS_PROJECTION} for
     *            the query
     * @return returns true if model was updated with the info in the cursor.
     */
    public static boolean setModelFromCalendarCursor(CalendarEventModel model,
            Cursor cursor) {
        ...
        while (cursor.moveToNext()) {
            ...
            model.mOrganizerCanRespond = cursor
                    .getInt(CALENDARS_INDEX_CAN_ORGANIZER_RESPOND) != 0;

            model.mCalendarAccessLevel = cursor
                    .getInt(CALENDARS_INDEX_ACCESS_LEVEL);
            model.mCalendarDisplayName = cursor
                    .getString(CALENDARS_INDEX_DISPLAY_NAME);
            model.setCalendarColor(Utils.getDisplayColorFromColor(cursor
                    .getInt(CALENDARS_INDEX_COLOR)));

            model.mCalendarAccountName = cursor
                    .getString(CALENDARS_INDEX_ACCOUNT_NAME);
            model.mCalendarAccountType = cursor
                    .getString(CALENDARS_INDEX_ACCOUNT_TYPE);

            model.mCalendarMaxReminders = cursor
                    .getInt(CALENDARS_INDEX_MAX_REMINDERS);
            model.mCalendarAllowedReminders = cursor
                    .getString(CALENDARS_INDEX_ALLOWED_REMINDERS);
    ...
    public static boolean canRespond(CalendarEventModel model) {
        ...
        // holiday and term event
        if (model.mEventType == EventItem.VIEW_EVENT_NORMAL) {
            return false;
        }
     ...
     * 
     * @param model
     *            The complete model of the event you want to save
     * @return values
     */
    ContentValues getContentValuesFromModel(CalendarEventModel model) {
        ...
        if (isAllDay) {
            ...
            if (endMillis < startMillis + DateUtils.DAY_IN_MILLIS) {
                // EditEventView#fillModelFromUI() should treat this case, but
                // we want to ensure
        ...
        values.put(Events.RRULE, rrule);
        values.put(Events.RDATE, model.mRdate);
        if (!TextUtils.isEmpty(rrule)) {
            addRecurrenceRule(values, model);
        } else if (!TextUtils.isEmpty(model.mRdate)) {
            addRecurrenceDate(values, model);
        } else {
        ...
        if (accessLevel > 0) {
            // For now the array contains the values 0, 2, and 3. We add one to
            // match.
        ...
        if (model.isEventColorInitialized()) {
            Log.d(TAG, "lxg model.getEventColor() =" + model.getEventColor()
                    + ",model.getCalendarColor() = " + model.getCalendarColor());
            if (model.getEventColor() == model.getCalendarColor()) {
                ...
                values.put(Events.EVENT_COLOR, model.getEventColor());
                // HCT_MODIFY,lixiange MF3.0 new event
            }
        }
        // add by hct.gengbin
        if (model.mEventType == EventItem.VIEW_EVENT_LOCATION) {
            values.put(EVENT_EXT_EVENT_TYPE, model.mEventType);
            values.put(EVENT_EXT_ACTION_DATA, model.mActionData);// use
            values.put(EVENT_EXT_ACTION_DATA2, model.mActionData2); // action_data
            // for location
            // remind
            // radius.
        }
        values.put(EVENT_EXT_ACTION_DATA1, model.mActionData1);// map params
        return values;
    }

    /**
     * If the recurrence rule is such that the event start date doesn't actually
     * fall in one of the recurrences, then push the start date up to the first
     * actual instance of the event.
     */
    private void offsetStartTimeIfNecessary(Time startTime, Time endTime,
            String rrule, CalendarEventModel model) {
        ...
        // In this case, we'll need to push the start time to fall on the first
        // day of the week
        ...
        if (mEventRecurrence.byday == null
            ...
            // This shouldn't happen, but just in case something is weird about
            // the recurrence.
            return;
        }
        ...
        for (int i = 0; i < mEventRecurrence.bydayCount; i++) {
            ...
            // We either want the earliest day that is later in the week than
            // startDay ...
            if (day > startDay
                    && (day < closestWeekday || closestWeekday < startDay)) {
                closestWeekday = day;
            }
            // ... or if there are no days later than startDay, we want the
            // earliest day that is
            // earlier in the week than startDay.
            if (closestWeekday == Integer.MAX_VALUE
                    || closestWeekday < startDay) {
                // We haven't found a day that's later in the week than startDay
                // yet.
        ...
        // We're here, so unfortunately our event's start day is not included in
        // the days of
        // the week of the recurrence. To save this event correctly we'll need
        // to push the start
        ...
        // Later we'll actually be using the values from the model rather than
        // the startTime
        // and endTime themselves, so we need to make these changes to the model
        // as well.
