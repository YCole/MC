...
import com.hct.calendar.lbs.LbsServiceHelper.CreateFenceResult;
...
public class EditEventFragment extends Fragment implements EventHandler,
    ...
    static final long STORAGE_WARNING_LONG = 2 * 1024 * 1024;

    private static final int TOKEN_ALL = TOKEN_EVENT | TOKEN_ATTENDEES
            | TOKEN_REMINDERS | TOKEN_CALENDARS | TOKEN_COLORS;
    private static final int TOKEN_UNITIALIZED = 1 << 31;

    private Button action_cancel_event;
    private Button action_done_event;
        ...
        protected void onQueryComplete(int token, Object cookie, Cursor cursor) {
            ...
            case TOKEN_EVENT:
                ...
                if (mModel.mHasAlarm && mReminders == null) {
                    ...
                    mModel.mReminders = (ArrayList<ReminderEntry>) mReminders
                            .clone();
                ...
                mHandler.startQuery(TOKEN_COLORS, null, Colors.CONTENT_URI,
                        EditEventHelper.COLORS_PROJECTION, Colors.COLOR_TYPE
            ...
            case TOKEN_ATTENDEES:
                try {
                    while (cursor.moveToNext()) {
                        String name = cursor
                                .getString(EditEventHelper.ATTENDEES_INDEX_NAME);
                        String email = cursor
                                .getString(EditEventHelper.ATTENDEES_INDEX_EMAIL);
                        int status = cursor
                                .getInt(EditEventHelper.ATTENDEES_INDEX_STATUS);
                        ...
                        if (email != null) {
                            if (mModel.mOwnerAccount != null
                                    && mModel.mOwnerAccount
                                            .equalsIgnoreCase(email)) {
                                int attendeeId = cursor
                                        .getInt(EditEventHelper.ATTENDEES_INDEX_ID);
            ...
            case TOKEN_REMINDERS:
                try {
                    ...
                    while (cursor.moveToNext()) {
                        ...
                        int method = cursor
                                .getInt(EditEventHelper.REMINDERS_INDEX_METHOD);
                        ReminderEntry re = ReminderEntry.valueOf(minutes,
                                method);
                ...
                try {
                    if (mModel.mId == -1) {
                        ...
                        if (matrixCursor == null) {
                            Log.d(TAG,
                                    "Got a closed or null cursor from onLoadComplete 3");
                            return;
                        }
                        if (DEBUG) {
                        ...
                        mView.setCalendarsCursor(matrixCursor, isAdded()
                                && isResumed(), mCalendarId);
                    } else {
                        // Populate model for an existing event
                        EditEventHelper.setModelFromCalendarCursor(mModel,
                                cursor);
            ...
            case TOKEN_COLORS:
                if (cursor.moveToFirst()) {
                    ...
                    do {
                        int colorKey = cursor
                                .getInt(EditEventHelper.COLORS_INDEX_COLOR_KEY);
                        int rawColor = cursor
                                .getInt(EditEventHelper.COLORS_INDEX_COLOR);
                    ...
                    // mView.mColorPickerNewEvent
                    // .setOnClickListener(mOnColorPickerClicked);
                    // mView.mColorPickerExistingEvent
                    // .setOnClickListener(mOnColorPickerClicked);
                }
                if (cursor != null) {
                    cursor.close();
    ...
    private View.OnClickListener mOnColorPickerClicked = new View.OnClickListener() {
        ...
        public void onClick(View v) {
            ...
            if (!mColorPickerDialog.isAdded()) {
                mColorPickerDialog.show(fragmentManager,
    ...
    public EditEventFragment(EventInfo event,
            ArrayList<ReminderEntry> reminders, boolean eventColorInitialized,
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        mColorPickerDialog = (EventColorPickerDialog) getActivity()
                .getFragmentManager()
    ...
    private void startQuery() {
        ...
        if (mEvent != null) {
            if (mEvent.id != -1) {
                ...
                mUri = ContentUris
                        .withAppendedId(Events.CONTENT_URI, mEvent.id);
            } else {
                // New event. All day?
            ...
            if (mEventBundle.id != -1) {
                ...
                mUri = ContentUris.withAppendedId(Events.CONTENT_URI,
                        mEventBundle.id);
            }
        ...
        /*
         * if (mBegin <= 0) { // use a default value instead mBegin =
         * mHelper.constructDefaultStartTime(System .currentTimeMillis()); }
         */
        if (mEnd < mBegin || mEnd == 0) {
        ...
        if (!newEvent) {
            ...
            mHandler.startQuery(TOKEN_EVENT, null, mUri,
                    EditEventHelper.EVENT_PROJECTION, null /* selection */,
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        mView = new EditEventView(mActivity, view, mOnDone,
                mTimeSelectedWasStartTime, mDateSelectedWasStartDate,
                mEventColor);
        ((ActivityHCT) getActivity())
                .addBottomBarOptionMenu(R.menu.edit_event_title_bar);
        startQuery();
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        if (savedInstanceState != null) {
            if (savedInstanceState.containsKey(BUNDLE_KEY_MODEL)) {
                mRestoreModel = (CalendarEventModel) savedInstanceState
                        .getSerializable(BUNDLE_KEY_MODEL);
            }
            if (savedInstanceState.containsKey(BUNDLE_KEY_EDIT_STATE)) {
                mModification = savedInstanceState
                        .getInt(BUNDLE_KEY_EDIT_STATE);
            }
            ...
            if (savedInstanceState.containsKey(BUNDLE_KEY_EVENT)) {
                mEventBundle = (EventBundle) savedInstanceState
                        .getSerializable(BUNDLE_KEY_EVENT);
            }
            if (savedInstanceState.containsKey(BUNDLE_KEY_READ_ONLY)) {
                mIsReadOnly = savedInstanceState
                        .getBoolean(BUNDLE_KEY_READ_ONLY);
            }
            if (savedInstanceState
                    .containsKey("EditEventView_timebuttonclicked")) {
                mTimeSelectedWasStartTime = savedInstanceState
                        .getBoolean("EditEventView_timebuttonclicked");
            }
            if (savedInstanceState.containsKey(BUNDLE_KEY_DATE_BUTTON_CLICKED)) {
                mDateSelectedWasStartDate = savedInstanceState
                        .getBoolean(BUNDLE_KEY_DATE_BUTTON_CLICKED);
            }
            if (savedInstanceState.containsKey(BUNDLE_KEY_SHOW_COLOR_PALETTE)) {
                mShowColorPalette = savedInstanceState
                        .getBoolean(BUNDLE_KEY_SHOW_COLOR_PALETTE);
            }
    ...
    // hct_modify lixiange MF3.0
    /**
     * if (!mUseCustomActionBar) { inflater.inflate(R.menu.edit_event_title_bar,
     * menu); }
     */
    /**
     * inflater.inflate(R.menu.edit_event_title_bar, menu); // hct_modify
     * lixiange MF3.0 }
     */
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.action_done_event) {
            long lAvailableFlash = getAvailableInternalMemorySize();
            Log.d(TAG, "lxg lAvailableFlash=" + lAvailableFlash);
            String str1 = mView.mLocationTextView.getText().toString();
            String MAP_DIR = mActivity.getFilesDir().getPath() + "/"
                    + "calendarmap";
            String newLocationMap = MAP_DIR + "/" + str1 + ".jpg";
            String locationMap = MAP_DIR + "/" + mView.mLocationTextString
                    + ".jpg";
            File targetFile = new File(locationMap);
            File newTargetFile = new File(newLocationMap);
            if (mView.mMap != null
                    && !TextUtils.isEmpty(mView.mLocationTextString)
                    && locationMap != null) {
                if (targetFile.exists()) {
                    targetFile.renameTo(newTargetFile);
                }
            }
            if (lAvailableFlash <= STORAGE_WARNING_LONG) {
                Log.d(TAG, "lxg low space");
                Toast.makeText(mActivity, R.string.flash_full,
        ...
        } else if (itemId == R.id.action_cancel_event) {
            mOnDone.setDoneCode(Utils.DONE_REVERT);
            mOnDone.run();
        }
        return true;
    ...
    private void saveReminders() {
        ArrayList<ContentProviderOperation> ops = new ArrayList<ContentProviderOperation>(
                3);
        boolean changed = EditEventHelper.saveReminders(ops, mModel.mId,
                mModel.mReminders, mOriginalModel.mReminders, false /*
                                                                     * no force
                                                                     * save
                                                                     */);
        ...
        Toast.makeText(mActivity, R.string.saving_event, Toast.LENGTH_SHORT)
                .show();
    }
    ...
    protected void displayEditWhichDialog() {
        if (mModification == Utils.MODIFY_UNINITIALIZED) {
            ...
            // list
            if (!isFirstEventInSeries) {
                items[itemIndex++] = mActivity
                        .getText(R.string.modify_all_following);
            }
                        ...
                        public void onClick(DialogInterface dialog, int which) {
                            if (which == 0) {
                                ...
                                if (mModification == Utils.MODIFY_SELECTED) {
                                    mModel.mOriginalSyncId = notSynced ? null
                                            : mModel.mSyncId;
        ...
        public void run() {
            ...
            if ((mCode & Utils.DONE_SAVE) != 0
                    ...
                    && (EditEventHelper.canRespond(mModel) || EditEventHelper
                            .canModifyEvent(mModel)) && mView.prepareForSave()
                    && !isEmptyNewEvent() && mModel.normalizeReminders()) {
                if (LbsServiceHelper.saveOrUpdateLocationFence(getActivity(),
                        mOriginalModel, mModel)) {
                    // sync to save
                    return;
                } else {
                    int frequence = mModel.mRepeatFrequence;
                    int duration = mView.eventDuration;
                    if (frequence != -1 && frequence < duration
                            && mModel.mRrule != null) {
                        Toast.makeText(mActivity,
                                mActivity.getString(R.string.event_alert_info),
                                Toast.LENGTH_SHORT).show();
                        return;
                    } else {
                        saveEvent();
                ...
                // add by hct.gengbin for location remind
            } else if ((mCode & Utils.DONE_SAVE) != 0 && mModel != null
                    && isEmptyNewEvent()) {
                Toast.makeText(mActivity, R.string.empty_event,
                        Toast.LENGTH_SHORT).show();
            }
    ...
    boolean isEmptyNewEvent() {
        ...
        /**
         * if (mModel.mOriginalStart != mModel.mStart || mModel.mOriginalEnd !=
         * mModel.mEnd) { return false; }
         * 
         * if (!mModel.mAttendeesList.isEmpty()) { return false; }
         */
    ...
    public void onPause() {
        /**
         * Activity act = getActivity(); if (mSaveOnDetach && act != null &&
         * !mIsReadOnly && !act.isChangingConfigurations() &&
         * mView.prepareForSave()) { mOnDone.setDoneCode(Utils.DONE_SAVE);
         * mOnDone.run(); }
         */
        super.onPause();
    }
    ...
    public void onAcitivityResultLocation(Intent data) {
        if (mView != null) {
            mView.addLocation(data, mActivity);
        }
    }

    public void onAcitivityResult(Intent data) {
        if (mView != null) {
            mView.addContactEmailAddress(data);
        }
    }
    ...
    public void onActivityResult(int requestCode, int resultCode, Intent data) {
        if (requestCode == 1000 && data != null) {
            mView.addMapLocation(data);
        }
        if (requestCode == 2000 && data != null) {
            CreateFenceResult result = LbsServiceHelper
                    .parseCreateFenceResult(data
                            .getStringExtra("createFenceResult"));
            if (result.status == 0) {
                mModel.mActionData2 = String.valueOf(result.fencenId);
                mModel.mReminders.clear();
                saveEvent();
            } else {
                Toast.makeText(mActivity, result.message, Toast.LENGTH_SHORT)
                        .show();
            }
        }
        if (requestCode == 101 && data != null) {
            mView.addEmailAddressFromContact(data);
        }
    ...
    public void onSaveInstanceState(Bundle outState) {
        ...
        outState.putBoolean(BUNDLE_KEY_EDIT_ON_LAUNCH,
        ...
        // outState.putBoolean(BUNDLE_KEY_SHOW_COLOR_PALETTE,
        // mView.isColorPaletteVisible());

        outState.putBoolean("EditEventView_timebuttonclicked",
                mView.mTimeSelectedWasStartTime);
        outState.putBoolean(BUNDLE_KEY_DATE_BUTTON_CLICKED,
                mView.mDateSelectedWasStartDate);
    }
    ...
    public void handleEvent(EventInfo event) {
        ...
        if ((false && event.eventType == EventType.USER_HOME)
                || (event.eventType == EventType.GO_TO && mSaveOnDetach)) {
            if (mView != null && mView.prepareForSave()) {
    ...
    private void popupdialog() {
        ...
        AlertDialog dialog = new AlertDialog.Builder(mActivity)
                .setMessage(R.string.back_pressed_dialog_message)
                .setTitle(R.string.back_pressed_dialog_title)
                .setNegativeButton(R.string.back_pressed_dialog_cancel,
                        new OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                    int which) {
                                Log.d(TAG, "lxg setNegativeButton");
                                act.finish();
                            }
                        })
                .setPositiveButton(R.string.back_pressed_dialog_save,
                        new OnClickListener() {
                            public void onClick(DialogInterface dialog,
                                    int which) {
                                if (mSaveOnDetach && act != null
                                        && !mIsReadOnly
                                        && !act.isChangingConfigurations()
                                        && mView.prepareForSave())
                                    Log.d(TAG, "lxg setPositiveButton");
                                mOnDone.setDoneCode(Utils.DONE_SAVE);
                                mOnDone.run();
                                act.finish();
                            }
