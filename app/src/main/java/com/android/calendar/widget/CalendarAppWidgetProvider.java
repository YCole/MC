...
public class CalendarAppWidgetProvider extends AppWidgetProvider {
     ...
     * 
     * @param context
     *            Context to use when starting {@link CalendarAppWidgetService}.
     * @param appWidgetIds
     *            List of specific appWidgetIds to update, or null for all.
     * @param changedEventIds
     *            Specific events known to be changed. If present, we use it to
    ...
    private void performUpdate(Context context,
        ...
        for (int appWidgetId : appWidgetIds) {
            ...
            long smillis = millis + 6 * 24 * 60 * 60 * 1000;
            final String dayOfWeek = DateUtils.getDayOfWeekString(
                    time.weekDay + 1, DateUtils.LENGTH_MEDIUM);
            /*
             * final String date = Utils.formatDateRange(context, millis,
             * millis, DateUtils.FORMAT_ABBREV_ALL | DateUtils.FORMAT_SHOW_DATE
             * | DateUtils.FORMAT_NO_YEAR); final String sdate =
             * Utils.formatDateRange(context, smillis, smillis,
             * DateUtils.FORMAT_ABBREV_ALL | DateUtils.FORMAT_SHOW_DATE |
             * DateUtils.FORMAT_NO_YEAR); final String ssdate = date + "-" +
             * sdate; Log.v(TAG, "ssdate=" + ssdate);
             * 
             * views.setTextViewText(R.id.day_of_week, ssdate);
             */
            // added by chenhuaiyu start
            String[] yearMonthWeekString = getYearMonthWeekString(context);
            views.setTextViewText(R.id.typeb_calendar_month,
                    yearMonthWeekString[0]);
            views.setTextViewText(R.id.typeb_calendar_top_week,
                    yearMonthWeekString[1]);
            views.setTextViewText(R.id.typeb_calendar_year,
                    yearMonthWeekString[2]);
            // added by chenhuaiyu end
            // Add new events
            ...
            PendingIntent pendingEditEventIntent = PendingIntent.getActivity(
                    context, 0, editEventIntent,
                    PendingIntent.FLAG_UPDATE_CURRENT);
            views.setOnClickPendingIntent(R.id.widget_agenda_add_linearlayout,
            ...
            launchCalendarIntent.setData(Uri
                    .parse("content://com.android.calendar/time/" + millis));
            final PendingIntent launchCalendarPendingIntent = PendingIntent
                    .getActivity(context, 0 /* no requestCode */,
                            launchCalendarIntent, 0 /* no flags */);
            views.setOnClickPendingIntent(R.id.header,
                    launchCalendarPendingIntent);
    ...
    /**
     * get month week year to show chy
     */
     ...
     * 
     * @param context
     *            Context to use when building broadcast.
     */
    static PendingIntent getUpdateIntent(Context context) {
        Intent intent = new Intent(
                Utils.getWidgetScheduledUpdateAction(context));
    ...
    /**
     * Build a {@link PendingIntent} to launch the Calendar app. This should be
     * used in combination with
     * {@link RemoteViews#setPendingIntentTemplate(int, PendingIntent)}.
     */
    ...
    /**
     * Build an {@link Intent} available as FillInIntent to launch the Calendar
     * app. This should be used in combination with
     * {@link RemoteViews#setOnClickFillInIntent(int, Intent)}. If the go to
     * time is 0, then calendar will be launched without a starting time.
     * 
     * @param goToTime
     *            time that calendar should take the user to, or 0 to indicate
     *            no specific start time.
     */
    static Intent getLaunchFillInIntent(Context context, long id, long start,
            long end, boolean allDay) {
        final Intent fillInIntent = new Intent();
        String dataString = "content://com.android.calendar/events";
        if (id != 0) {
            // start remove for bugID:92980
            // fillInIntent.putExtra(Utils.INTENT_KEY_DETAIL_VIEW, true);
            // end remove for bugID:92980
            ...
            Log.d("zyp", "zyp -----------   :" + id);
            fillInIntent.setClass(context, ScheduleDetailsActivity.class);
        } else {
