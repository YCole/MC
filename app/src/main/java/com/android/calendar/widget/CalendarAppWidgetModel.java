...
class CalendarAppWidgetModel {
    private static final String TAG = CalendarAppWidgetModel.class
            .getSimpleName();
    ...
    private final long tenSixHours = 16 * 60 * 60 * 1000;
    ...
    static class EventInfo {
        int visibWhen; // Visibility value for When textview (View.GONE or
                       // View.VISIBLE)
        String when;
        int visibWhere; // Visibility value for Where textview (View.GONE or
                        // View.VISIBLE)
        String where;
        int visibTitle; // Visibility value for Title textview (View.GONE or
                        // View.VISIBLE)
        String title;
    ...
    static class DayInfo {
        ...
        public int hashCode() {
            ...
            int result = 1;
            result = prime * result
                    + ((mDayLabel == null) ? 0 : mDayLabel.hashCode());
    ...
    public void buildFromCursor(Cursor cursor, String timeZone) {
        ...
        final ArrayList<LinkedList<RowInfo>> mBuckets = new ArrayList<LinkedList<RowInfo>>(
        ...
        if (mShowTZ) {
            mHomeTZName = TimeZone.getTimeZone(timeZone).getDisplayName(
                    recycle.isDst != 0, TimeZone.SHORT);
        }
        ...
        while (cursor.moveToNext()) {
            ...
            final long eventId = cursor
                    .getLong(CalendarAppWidgetService.INDEX_EVENT_ID);
            final boolean allDay = cursor
                    .getInt(CalendarAppWidgetService.INDEX_ALL_DAY) != 0;
            ...
            final String title = cursor
                    .getString(CalendarAppWidgetService.INDEX_TITLE);
            final String location = cursor
                    .getString(CalendarAppWidgetService.INDEX_EVENT_LOCATION);
            // we don't compute these ourselves because it seems to produce the
            // wrong endDay for all day events
            final int startDay = cursor
                    .getInt(CalendarAppWidgetService.INDEX_START_DAY);
            final int endDay = cursor
                    .getInt(CalendarAppWidgetService.INDEX_END_DAY);
            final int color = cursor
                    .getInt(CalendarAppWidgetService.INDEX_COLOR);
            ...
            if (allDay) {
                ...
                // change start by zyp for GMOS-1001 . I need add 24 hours to
                // end and ensur end time is night 24:00
                end = end + 24 * 60 * 60 * 1000;
                // change end by zyp for GMOS-1001 . I need add 24 hours to end
                // and ensur end time is night 24:00
            }

            if (LOGD) {
                Log.d(TAG, "Row #" + rowId + " allDay:" + allDay + " start:"
                        + start + " end:" + end + " eventId:" + eventId);
            }
            ...
            mEventInfos.add(populateEventInfo(eventId, allDay, start, end,
                    startDay, endDay, title, location, color, selfStatus));
            ...
            for (int day = from; day <= to; day++) {
                LinkedList<RowInfo> bucket = mBuckets
                        .get(day - mTodayJulianDay);
        ...
        for (LinkedList<RowInfo> bucket : mBuckets) {
            if (!bucket.isEmpty()) {
                // We don't show day header in today------>no,we should show.
    ...
    private EventInfo populateEventInfo(long eventId, boolean allDay,
            long start, long end, int startDay, int endDay, String title,
            String location, int color, int selfStatus) {
        ...
        if (allDay) {
            ...
            whenString.append(mContext
                    .getString(R.string.edit_event_all_day_label));
            // whenString.append(Utils.formatDateRange(mContext, start, end,
            // flags));
        } else {
            ...
            whenString.append(Utils
                    .formatDateRange(mContext, start, end, flags));
    ...
    private DayInfo populateDayInfo(int julianDay, Time recycle) {
        ...
        int flagsf = DateUtils.FORMAT_ABBREV_ALL | DateUtils.FORMAT_SHOW_DATE;

        String label, labelf, labels;
        // if (julianDay == mTodayJulianDay){
        labelf = Utils.formatDateRange(mContext, millis, millis, flagsf);
        int flagss = DateUtils.FORMAT_SHOW_WEEKDAY;
        labels = Utils.formatDateRange(mContext, millis, millis, flagss);
        label = labelf + " , " + labels;
        // label = mContext.getString(R.string.agenda_today,
        // Utils.formatDateRange(mContext, millis, millis, flags).toString());
        /*
         * } if (julianDay == mTodayJulianDay + 1) { flags |=
         * DateUtils.FORMAT_SHOW_WEEKDAY; label =
         * Utils.formatDateRange(mContext, millis, millis, flags); //label =
         * mContext.getString(R.string.agenda_tomorrow, //
         * Utils.formatDateRange(mContext, millis, millis, flags).toString()); }
         * else { flags |= DateUtils.FORMAT_SHOW_WEEKDAY; label =
