...
public class CalendarAppWidgetService extends RemoteViewsService {
    ...
    private static final String EVENT_SORT_ORDER = Instances.START_DAY
            + " ASC, " + Instances.START_MINUTE + " ASC, " + Instances.END_DAY
            + " ASC, " + Instances.END_MINUTE + " ASC LIMIT " + EVENT_MAX_COUNT;
    ...
    static final String[] EVENT_PROJECTION = new String[] { Instances.ALL_DAY,
            Instances.BEGIN, Instances.END, Instances.TITLE,
            Instances.EVENT_LOCATION, Instances.EVENT_ID, Instances.START_DAY,
            Instances.END_DAY, Instances.DISPLAY_COLOR, // If SDK < 16, set to
    ...
    private static final long SEARCH_DURATION = MAX_DAYS
            * DateUtils.DAY_IN_MILLIS;

    /**
     * Update interval used when no next-update calculated, or bad trigger time
     * in past. Unit: milliseconds.
     */
    ...
    public static class CalendarFactory extends BroadcastReceiver implements
        ...
        // Suppress unnecessary logging about update time. Need to be static as
        // this object is
        // re-instanciated frequently.
        // TODO: It seems loadData() is called via onCreate() four times, which
        // should mean
        // unnecessary CalendarFactory object is created and dropped. It is not
        // efficient.
        ...
        private int mDeclinedColor2;
        private int mStandardColor2;
        private int mAllDayColor;

        private boolean mEventViewLoaded = false;
        ...
        protected CalendarFactory(Context context, Intent intent) {
            ...
            mDeclinedColor = mResources.getColor(R.color.text_color);
            mStandardColor = mResources.getColor(R.color.text_color);
            mDeclinedColor2 = mResources
                    .getColor(R.color.cale_week_title_color);
            mStandardColor2 = mResources
                    .getColor(R.color.cale_week_title_color);
            mAllDayColor = mResources
                    .getColor(R.color.appwidget_item_allday_color);
        }
        ...
        public RemoteViews getViewAt(int position) {
            // we use getCount here so that it doesn't return null when
            // empty
            ...
            if (mModel == null) {
                ...
                final Intent intent = CalendarAppWidgetProvider
                        .getLaunchFillInIntent(mContext, 0, 0, 0, false);
                Log.d("zyp", "zyp ------1111----    :");
            ...
            if ((mModel.mEventInfos.isEmpty() || mModel.mRowInfos.isEmpty())
                ...
                return views;
            } else if (!mModel.mEventInfos.isEmpty()
                    && !mModel.mRowInfos.isEmpty()) {
                RowInfo rowInfo = mModel.mRowInfos.get(position);

                if (rowInfo.mType == RowInfo.TYPE_DAY) {
                    RemoteViews views = new RemoteViews(
                            mContext.getPackageName(), R.layout.appwidget_day);
                    if (position > 0) {
                        views.setViewVisibility(R.id.appwidget_list,
                                View.VISIBLE);
                    } else {
                        views.setViewVisibility(R.id.appwidget_list, View.GONE);
                    }
                    DayInfo dayInfo = mModel.mDayInfos.get(rowInfo.mIndex);
                    updateTextView(views, R.id.date, View.VISIBLE, mContext
                            .getResources().getString(R.string.today));
                    return views;
                } else {
                    RemoteViews views;
                    final EventInfo eventInfo = mModel.mEventInfos
                            .get(rowInfo.mIndex);
                    /*
                     * if (eventInfo.allDay) { views = new
                     * RemoteViews(mContext.getPackageName(),
                     * R.layout.widget_item); } else {
                     */
                    views = new RemoteViews(mContext.getPackageName(),
                            R.layout.widget_item);
                    if (position + 1 == mModel.mRowInfos.size()) {
                        // views.setViewVisibility(R.id.widget_event_divider,
                        // View.GONE);
                        views.setViewVisibility(R.id.widget_event_divider_last,
                                View.VISIBLE);
                    } else if (position + 1 < mModel.mRowInfos.size()) {
                        RowInfo rowInfonext = mModel.mRowInfos
                                .get(position + 1);
                        if (rowInfonext.mType == RowInfo.TYPE_DAY) {
                            // views.setViewVisibility(R.id.widget_event_divider,
                            // View.GONE);
                        } else {
                            views.setViewVisibility(R.id.widget_event_divider,
                    ...
                    int displayColor = Utils
                            .getDisplayColorFromColor(eventInfo.color);

                    final long now = System.currentTimeMillis();
                    /*
                     * if (eventInfo.start <= now && now <= eventInfo.end) {
                     * views.setInt(R.id.widget_row, "setBackgroundResource",
                     * R.drawable.agenda_item_bg_secondary); } else {
                     * views.setInt(R.id.widget_row, "setBackgroundResource",
                    ...
                    if (selfAttendeeStatus == Attendees.ATTENDEE_STATUS_INVITED) {
                        ...
                        views.setInt(R.id.when, "setTextColor", mStandardColor2);
                        views.setInt(R.id.where, "setTextColor",
                                mStandardColor2);
                    }
                    ...
                    /*
                     * } else if (selfAttendeeStatus ==
                     * Attendees.ATTENDEE_STATUS_DECLINED) {
                     * views.setInt(R.id.title, "setTextColor", mDeclinedColor);
                     * views.setInt(R.id.when, "setTextColor", mDeclinedColor2);
                     * views.setInt(R.id.where, "setTextColor",
                     * mDeclinedColor2); // views.setInt(R.id.agenda_item_color,
                     * "setDrawStyle", // ColorChipView.DRAW_CROSS_HATCHED);
                     * views.setInt(R.id.agenda_item_color, "setImageResource",
                     * R.drawable.widget_chip_responded_bg); // 40% opacity
                     * views.setInt(R.id.agenda_item_color, "setColorFilter",
                     * Utils.getDeclinedColorFromColor(displayColor)); } else {
                     * views.setInt(R.id.title, "setTextColor", mStandardColor);
                     * views.setInt(R.id.when, "setTextColor", mStandardColor2);
                     * views.setInt(R.id.where, "setTextColor",
                     * mStandardColor2); if (selfAttendeeStatus ==
                     * Attendees.ATTENDEE_STATUS_INVITED) {
                     * views.setInt(R.id.agenda_item_color, "setImageResource",
                     * R.drawable.widget_chip_not_responded_bg); } else {
                     * views.setInt(R.id.agenda_item_color, "setImageResource",
         ...
         * 
         * @param selection
         *            The selection string for the loader to filter the query
         *            with.
         */
        public void initLoader(String selection) {
            Log.d("liaozhenbin", selection);
            Log.d("liaozhenbin", EVENT_PROJECTION[0]);
            Log.d("liaozhenbin", EVENT_SORT_ORDER);
            if (LOGD)
        ...
        /**
         * This gets the selection string for the loader. This ends up doing a
         * query in the shared preferences.
         */
        ...
        private long calculateUpdateTime(CalendarAppWidgetModel model,
            ...
            for (EventInfo event : model.mEventInfos) {
                ...
                // We want to update widget when we enter/exit time range of an
                // event.
                if (now < start) {
        ...
        public void onLoadComplete(Loader<Cursor> loader, Cursor cursor) {
            ...
            synchronized (mLock) {
                ...
                if (matrixCursor == null) {
                    Log.d(TAG,
                            "Got a closed or null cursor from onLoadComplete 2");
                    return;
                }
                try {
                ...
                if (triggerTime < now) {
                    Log.w(TAG, "Encountered bad trigger time "
                ...
                if (mAppWidgetId == -1) {
                    int[] ids = widgetManager
                            .getAppWidgetIds(CalendarAppWidgetProvider
        ...
        public void onReceive(Context context, Intent intent) {
            if (LOGD)
                Log.d(TAG, "AppWidgetService received an intent. It was "
                        + intent.toString());
            mContext = context;

            // We cannot do any queries from the UI thread, so push the
            // 'selection' query
            // to a background thread. However the implementation of the latter
            // query
            // (cursor loading) uses CursorLoader which must be initiated from
            // the UI thread,
            // so there is some convoluted handshaking here.
            //
            // Note that as currently implemented, this must run in a single
            // threaded executor
            // or else the loads may be run out of order.
            //
            // TODO: Remove use of mHandler and CursorLoader, and do all the
            // work synchronously
            // in the background thread. All the handshaking going on here
            // between the UI and
            // background thread with using goAsync, mHandler, and CursorLoader
            // is confusing.
     ...
     * @param now
     *            Current system time from {@link System#currentTimeMillis()}
