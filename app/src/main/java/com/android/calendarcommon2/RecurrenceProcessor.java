...
public class RecurrenceProcessor {
     ...
     * repeat forever, then 0 is returned.
     * 
     ...
     */
    public long getLastOccurence(Time dtstart, RecurrenceSet recur)
            throws DateException {
     ...
     *            the time of the first occurrence
     * @param maxtime
     *            the max possible time of the last occurrence. null means no
     *            limit
     * @param recur
     *            the recurrence
     * @return an estimate of the time (in UTC milliseconds) of the last
    ...
    public long getLastOccurence(Time dtstart, Time maxtime, RecurrenceSet recur)
        ...
        } else if (recur.rdates != null && recur.exrules == null
                && recur.exdates == null) {
        ...
        if (hasCount || recur.rdates != null || maxtime != null) {
            ...
            long[] dates = expand(dtstart, recur,
                    ...
                    (maxtime != null) ? maxtime.toMillis(false /* use isDst */)
                            : -1 /* range end */);
     ...
     * than or equal to the period of the FREQ.
     * 
     * Returns 0 if the event should not be filtered out Returns something else
     * (a rule number which is useful for debugging) if the event should not be
     * returned
     */
                ...
                for (int i = r.bydayCount - 1; i >= 0; i--) {
                    if (r.bydayNum[i] != 0) {
                        if (Log.isLoggable(TAG, Log.VERBOSE)) {
                            Log.v(TAG,
                                    "BYSETPOS not supported with these rules: "
                                            + r);
                        }
                        break bysetpos;
                    }
    ...
    private static boolean filterMonthlySetPos(EventRecurrence r, Time instance) {
         ...
         * 1st from that. Note DotW is 0-6, where 0=SUNDAY.
         * 
         * The basic calculation is to take the instance's "day of the week"
         * number, subtract (day of the month - 1) mod 7, and then make sure
         * it's positive. We can simplify that with some algebra.
         */
        ...
        boolean get(Time iterator, int day) {
            ...
            if (SPEW) {
                Log.i(TAG, "get called with iterator=" + iterator + " "
                        + iterator.month + "/" + iterator.monthDay + "/"
                ...
                if (t == null) {
                    ...
                    if (SPEW) {
                        Log.i(TAG, "set t=" + t + " " + t.month + "/"
                                + t.monthDay + "/" + t.year + " realMonth="
                                + realMonth + " mMonth=" + mMonth);
                    }
                }
                mYear = realYear;
                mMonth = realMonth;
     ...
     *            the non-inclusive end of the range to expand, in UTC
     *            milliseconds; use -1 for the entire range.
     ...
     */
    public long[] expand(Time dtstart, RecurrenceSet recur,
            long rangeStartMillis, long rangeEndMillis) throws DateException {
        ...
        if (recur.rrules != null) {
            for (EventRecurrence rrule : recur.rrules) {
                expand(dtstart, rrule, rangeStartDateValue, rangeEndDateValue,
                        true /* add */, dtSet);
            }
        }
        ...
        if (recur.exrules != null) {
            for (EventRecurrence exrule : recur.exrules) {
                expand(dtstart, exrule, rangeStartDateValue, rangeEndDateValue,
                        false /* remove */, dtSet);
            }
        }
     ...
     * defined in the local timezone of the event.
     * 
     * If you want all of the events, pass Long.MAX_VALUE for rangeEndDateValue.
     * If you pass Long.MAX_VALUE for rangeEnd, and the event doesn't have a
     * COUNT or UNTIL field, you'll get a DateException.
     * 
     * @param dtstart
     *            the dtstart date as defined in RFC2445. This {@link Time}
     ...
     *            the last date-time you care about, not inclusive (so if you
     *            care about everything up through and including Dec 22 1995,
     *            set last to Dec 23, 1995 00:00:00
     * @param add
     ...
     */
    public void expand(Time dtstart, EventRecurrence r,
            long rangeStartDateValue, long rangeEndDateValue, boolean add,
        ...
        try {
            ...
            if (rangeEndDateValue == Long.MAX_VALUE && r.until == null
                    && r.count == 0) {
            ...
            boolean usebyhour = useBYX(freq, EventRecurrence.HOURLY,
                    byhourCount);
            int byminuteCount = r.byminuteCount;
            boolean usebyminute = useBYX(freq, EventRecurrence.MINUTELY,
                    byminuteCount);
            int bysecondCount = r.bysecondCount;
            boolean usebysecond = useBYX(freq, EventRecurrence.SECONDLY,
                    bysecondCount);
            ...
            events: {
                while (true) {
                    int monthIndex = 0;
                    if (failsafe++ > MAX_ALLOWED_ITERATIONS) { // Give up after
                                                               // about 1 second
                                                               // of processing
                        throw new DateException("Recurrence processing stuck: "
                                + r.toString());
                    }
                    ...
                    do { // month
                        int month = usebymonth ? r.bymonth[monthIndex]
                                : iteratorMonth;
                        month--;
                        if (SPEW)
                        ...
                        if (useDays) {
                            ...
                            if (freq == EventRecurrence.WEEKLY) {
                                 ...
                                 * later on.)
                                 * 
                                 * The day that starts the week is determined by
                                 * WKST, which defaults to MO.
                                 * 
                        ...
                        do { // day
                            ...
                            int hourIndex = 0;
                            do {
                                int hour = usebyhour ? r.byhour[hourIndex]
                                        : iteratorHour;
                                if (SPEW)
                                    Log.i(TAG, "      hour=" + hour
                                            + " usebyhour=" + usebyhour);
                                ...
                                do {
                                    ...
                                    do {
                                        ...
                                        // days.
                                        generated.set(second, minute, hour,
                                                day, month, iteratorYear);
                                        ...
                                        if (genDateValue >= dtstartDateValue) {
                                            ...
                                            if (0 == filtered) {
                                                ...
                                                if (!(dtstartDateValue == genDateValue
                                                        && add
                                                        && dtstartDateValue >= rangeStartDateValue && dtstartDateValue < rangeEndDateValue)) {
                                                    ++count;
                                                }
                                        ...
                                        secondIndex++;
                                            ...
                                            && secondIndex < bysecondCount);
                                    minuteIndex++;
                                } while (usebyminute
                                        && minuteIndex < byminuteCount);
                                hourIndex++;
            ...
            Log.w(TAG, "DateException with r=" + r + " rangeStart="
                    + rangeStartDateValue + " rangeEnd=" + rangeEndDateValue);
            throw e;
        } catch (RuntimeException t) {
            Log.w(TAG, "RuntimeException with r=" + r + " rangeStart="
                    + rangeStartDateValue + " rangeEnd=" + rangeEndDateValue);
            throw t;
        }
    }
