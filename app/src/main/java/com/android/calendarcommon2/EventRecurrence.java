...
public class EventRecurrence {
     ...
     *             Thrown if the day argument is not one of the defined day
     *             constants.
     */
    ...
    private static void appendNumbers(StringBuilder s, String label, int count,
            int[] values) {
        if (count > 0) {
            s.append(label);
            count--;
     ...
     */
    private static boolean arraysEqual(int[] array1, int count1, int[] array2,
            int count2) {
        if (count1 != count2) {
            return false;
        }
    ...
    public boolean equals(Object obj) {
        ...
        return (startDate == null ? er.startDate == null : Time.compare(
                startDate, er.startDate) == 0)
                && freq == er.freq
                && (until == null ? er.until == null : until.equals(er.until))
                && count == er.count
                && interval == er.interval
                && wkst == er.wkst
                && arraysEqual(bysecond, bysecondCount, er.bysecond,
                        er.bysecondCount)
                && arraysEqual(byminute, byminuteCount, er.byminute,
                        er.byminuteCount)
                ...
                && arraysEqual(bymonthday, bymonthdayCount, er.bymonthday,
                        er.bymonthdayCount)
                && arraysEqual(byyearday, byyeardayCount, er.byyearday,
                        er.byyeardayCount)
                && arraysEqual(byweekno, byweeknoCount, er.byweekno,
                        er.byweeknoCount)
                && arraysEqual(bymonth, bymonthCount, er.bymonth,
                        er.bymonthCount)
                && arraysEqual(bysetpos, bysetposCount, er.bysetpos,
                        er.bysetposCount);
    }
    ...
    private void resetFields() {
        until = null;
        freq = count = interval = bysecondCount = byminuteCount = byhourCount = bydayCount = bymonthdayCount = byyeardayCount = byweeknoCount = bymonthCount = bysetposCount = 0;
    }
    ...
    public void parse(String recur) {
         ...
         * 
         * recur = "FREQ"=freq *( ; either UNTIL or COUNT may appear in a
         * 'recur', ; but UNTIL and COUNT MUST NOT occur in the same 'recur'
         * 
         ...
         * 
         * Examples: FREQ=MONTHLY;INTERVAL=2;COUNT=10;BYDAY=1SU,-1SU
         ...
         * parse the <value> into an output field.
         * 
         * By keeping track of which names we've seen in a bit vector, we can
         * verify the constraints indicated above (FREQ appears first, none of
         * them appear more than once -- though x-[name] would require special
         * treatment), and we have either UNTIL or COUNT but not both.
         * 
         * In general, RFC 2445 property names (e.g. "FREQ") and enumerations
         * ("TU") must be handled in a case-insensitive fashion, but case may be
         * significant for other properties. We don't have any case-sensitive
         * values in RRULE, except possibly for the custom "X-" properties, but
         * we ignore those anyway. Thus, we can trivially convert the entire
         * string to upper case and then use simple comparisons.
         * 
         * Differences from previous version: - allows lower-case property and
         * enumeration values [optional] - enforces that FREQ appears first -
         * enforces that only one of UNTIL and COUNT may be specified - allows
         * (but ignores) X-* parts - improved validation on various values (e.g.
         * UNTIL timestamps) - error messages are more specific
         * 
         * TODO: enforce additional constraints listed in RFC 5545, notably the
         * "N/A" entries in section 3.3.10. For example, if FREQ=WEEKLY, we
         * should reject a rule that includes a BYMONTHDAY part.
         */
        ...
        for (String part : parts) {
            ...
            if (parser == null) {
                ...
                throw new InvalidFormatException("Couldn't find parser for "
                        + lhs);
            } else {
                int flag = parser.parsePart(rhs, this);
                if ((parseFlags & flag) != 0) {
                    throw new InvalidFormatException("Part " + lhs
                            + " was specified twice");
                }
                parseFlags |= flag;
            }
    ...
    abstract static class PartParser {
         ...
         * @param er
         *            The EventRecurrence into which the result is stored.
         ...
         *         elements.
         */
        public static int[] parseNumberList(String listStr, int minVal,
                int maxVal, boolean allowZero) {
            int[] values;
            ...
            if (listStr.indexOf(",") < 0) {
                ...
                values = new int[len];
                for (int i = 0; i < len; i++) {
                    values[i] = parseIntRange(valueStrs[i], minVal, maxVal,
                            allowZero);
                }
            }
            return values;
        ...
        public int parsePart(String value, EventRecurrence er) {
            if (VALIDATE_UNTIL) {
                try {
                    ...
                    throw new InvalidFormatException("Invalid UNTIL value: "
                            + value);
                }
            }
            er.until = value;
    ...
    private static class ParseCount extends PartParser {
        @Override
        public int parsePart(String value, EventRecurrence er) {
            er.count = parseIntRange(value, Integer.MIN_VALUE,
    ...
    private static class ParseInterval extends PartParser {
        @Override
        public int parsePart(String value, EventRecurrence er) {
            er.interval = parseIntRange(value, Integer.MIN_VALUE,
                    Integer.MAX_VALUE, true);
            if (er.interval < 1) {
                Log.d(TAG, "Invalid Interval. Forcing INTERVAL to 1 from "
                        + value);
                er.interval = 1;
            }
    ...
    private static class ParseByDay extends PartParser {
        ...
        private static void parseWday(String str, int[] byday, int[] bydayNum,
                int index) {
    ...
    private static class ParseBySetPos extends PartParser {
        @Override
        public int parsePart(String value, EventRecurrence er) {
            int[] bysetpos = parseNumberList(value, Integer.MIN_VALUE,
                    Integer.MAX_VALUE, true);
