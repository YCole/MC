...
public class RecurrenceSet {
    ...
    /**
     * Creates a new RecurrenceSet from information stored in the events table
     * in the CalendarProvider.
     * 
     * @param values
     *            The values retrieved from the Events table.
     */
     ...
     * {@link Cursor} pointing to the events table in the CalendarProvider. The
     * cursor must contain the RRULE, RDATE, EXRULE, and EXDATE columns.
     * 
     * @param cursor
    ...
    public RecurrenceSet(Cursor cursor)
        ...
        int exruleColumn = cursor
                .getColumnIndex(CalendarContract.Events.EXRULE);
        int exdateColumn = cursor
                .getColumnIndex(CalendarContract.Events.EXDATE);
    ...
    public RecurrenceSet(String rruleStr, String rdateStr, String exruleStr,
            String exdateStr) throws EventRecurrence.InvalidFormatException {
        init(rruleStr, rdateStr, exruleStr, exdateStr);
    }

    private void init(String rruleStr, String rdateStr, String exruleStr,
            String exdateStr) throws EventRecurrence.InvalidFormatException {
        ...
        for (int i = 0; i < n; ++i) {
            ...
            try {
                ...
                throw new EventRecurrence.InvalidFormatException(
                        "TimeFormatException thrown when parsing time "
                                + rawDates[i] + " in recurrence " + recurrence);
     ...
     * 
     * @param component
     *            The iCalendar component containing the desired recurrence
     *            specification.
     * @param values
     *            The db values that should be updated.
     * @return true if the component contained the necessary information to
     *         specify a recurrence. The required fields are DTSTART, one of
     *         DTEND/DURATION, and one of RRULE/RDATE. Returns false if there
     *         was an error, including if the date is out of range.
     */
    public static boolean populateContentValues(ICalendar.Component component,
            ContentValues values) {
        try {
            ICalendar.Property dtstartProperty = component
                    .getFirstProperty("DTSTART");
            String dtstart = dtstartProperty.getValue();
            ICalendar.Parameter tzidParam = dtstartProperty
            ...
            // We force TimeZone to UTC for "all day recurring events" as the
            // server is sending no
            ...
            if ((TextUtils.isEmpty(dtstart))
                ...
                if (false) {
                    Log.d(TAG, "Recurrence missing DTSTART, DTEND/DURATION, "
                            + "or RRULE/RDATE: " + component.toString());
                }
                return false;
            }
            ...
            if (millis == -1) {
                if (false) {
                    Log.d(TAG,
                            "DTSTART is out of range: " + component.toString());
                }
                return false;
            }
    ...
    public static boolean populateComponent(Cursor cursor,
        ...
        int exdateColumn = cursor
                .getColumnIndex(CalendarContract.Events.EXDATE);
        int allDayColumn = cursor
                .getColumnIndex(CalendarContract.Events.ALL_DAY);
        ...
        if ((dtstart == -1)
                || (TextUtils.isEmpty(duration))
                || ((TextUtils.isEmpty(rruleStr)) && (TextUtils
                        .isEmpty(rdateStr)))) {
            // no recurrence.
            return false;
        }
    ...
    public static boolean populateComponent(ContentValues values,
        ...
        final String duration = values
                .getAsString(CalendarContract.Events.DURATION);
        final String tzid = values
                .getAsString(CalendarContract.Events.EVENT_TIMEZONE);
        final String rruleStr = values
                .getAsString(CalendarContract.Events.RRULE);
        final String rdateStr = values
                .getAsString(CalendarContract.Events.RDATE);
        final String exruleStr = values
                .getAsString(CalendarContract.Events.EXRULE);
        final String exdateStr = values
                .getAsString(CalendarContract.Events.EXDATE);
        final Integer allDayInteger = values
                .getAsInteger(CalendarContract.Events.ALL_DAY);
        final boolean allDay = (null != allDayInteger) ? (allDayInteger == 1)
                : false;
    ...
    public static void addPropertiesForRuleStr(ICalendar.Component component,
            String propertyName, String ruleStr) {
    ...
    private static final Pattern IGNORABLE_ICAL_WHITESPACE_RE = Pattern
            .compile("(?:\\r\\n?|\\n)[ \t]");
    ...
    public static String unfold(String foldedIcalContent) {
        return IGNORABLE_ICAL_WHITESPACE_RE.matcher(foldedIcalContent)
                .replaceAll("");
    }

    public static void addPropertyForDateStr(ICalendar.Component component,
            String propertyName, String dateStr) {
    ...
    private static String computeDuration(Time start,
        ...
        ICalendar.Property durationProperty = component
        ...
        ICalendar.Parameter endTzidParameter = dtendProperty
                .getFirstParameter("TZID");
        String endTzid = (endTzidParameter == null) ? start.timezone
                : endTzidParameter.value;
        ...
        if (start.allDay && (durationSeconds % 86400) == 0) {
            return "P" + (durationSeconds / 86400) + "D"; // Server wants this
                                                          // instead of P86400S
        } else {
