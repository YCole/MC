...
public class TimeZoneData {
    ...
    public TimeZoneData(Context context, String defaultTimeZoneId,
            long timeMillis) {
        mContext = context;
        is24HourFormat = TimeZoneInfo.is24HourFormat = DateFormat
        ...
        mPalestineDisplayName = context.getResources().getString(
                R.string.palestine_display_name);

        loadTzs(context);

        Log.i(TAG,
                "Time to load time zones (ms): "
    ...
    void loadTzs(Context context) {
        ...
        for (String tzId : tzIds) {
            ...
            if (getIdenticalTimeZoneInTheCountry(tzInfo) == -1) {
                if (DEBUG) {
                    Log.e(TAG,
                            "# Adding time zone from getAvailId: "
                                    + tzInfo.toString());
                }
        ...
        mTimeZonesByOffsets = new SparseArray<ArrayList<Integer>>(
                mHasTimeZonesInHrOffset.length);
        ...
        for (TimeZoneInfo tz : mTimeZones) {
            ...
            if (tz.mDisplayName == null) {
                tz.mDisplayName = tz.mTz.getDisplayName(
                        tz.mTz.inDaylightTime(date), TimeZone.LONG,
                        defaultLocal);
            }
    ...
    private void populateDisplayNameOverrides(Resources resources) {
        ...
        String[] labels = resources
                .getStringArray(R.array.timezone_rename_labels);

        int length = ids.length;
        if (ids.length != labels.length) {
            Log.e(TAG, "timezone_rename_ids len=" + ids.length
                    + " timezone_rename_labels len=" + labels.length);
    ...
    private void indexByOffsets(int idx, TimeZoneInfo tzi) {
        ...
        int index = OFFSET_ARRAY_OFFSET
                + (int) (offsetMillis / DateUtils.HOUR_IN_MILLIS);
        ...
        try {
            ...
            while ((line = reader.readLine()) != null) {
                ...
                if (!line.startsWith("#") && line.length() > 0) {
                    ...
                    if (DEBUG) {
                        Log.e(TAG,
                                "# Dropping identical time zone from backward: "
                                        + oldTzId);
                    }

                    // Remember the cooler/newer time zone id
                    if (mDefaultTimeZoneId != null
                            && mDefaultTimeZoneId.equals(oldTzId)) {
                ...
                if (!line.startsWith("#")) { // Skip comment lines
                    ...
                    if (mDefaultTimeZoneId != null
                        ...
                        TimeZone defaultTz = TimeZone
                                .getTimeZone(mDefaultTimeZoneId);
                        if (defaultTz != null) {
                            mDefaultTimeZoneInfo = new TimeZoneInfo(defaultTz,
                                    country);
                            ...
                            if (tzToOverride == -1) {
                                ...
                                mTimeZones.add(tzToOverride,
                                        mDefaultTimeZoneInfo);
                                if (DEBUG) {
                                    TimeZoneInfo tzInfoToOverride = mTimeZones
                                            .get(tzToOverride);
                    ...
                    if (identicalTzIdx == -1) {
                        ...
                        if (DEBUG) {
                            Log.e(TAG, "# Dropping identical time zone: "
                                    + timeZoneId + " ## " + tz.getDisplayName());
                        }
                    }
                    processedTimeZones.add(timeZoneId);
                }
    ...
    private String getCountryNames(String lang, String countryCode) {
        ...
        if (PALESTINE_COUNTRY_CODE.equalsIgnoreCase(countryCode)) {
            ...
            countryDisplayName = new Locale(lang, countryCode)
                    .getDisplayCountry(defaultLocale);
        }
        ...
        if (mBackupCountryCodes == null
                || !defaultLocale.equals(mBackupCountryLocale)) {
        ...
        int length = Math.min(mBackupCountryCodes.length,
                mBackupCountryNames.length);
