...
public class TimeZonePickerView extends LinearLayout implements TextWatcher,
    ...
    public TimeZonePickerView(Context context, AttributeSet attrs,
        ...
        mFilterAdapter = new TimeZoneFilterTypeAdapter(mContext, tzd,
                mResultAdapter);
        ...
        updateHint(R.string.hint_time_zone_search,
                R.drawable.ic_search_holo_light);
    ...
    public String getLastFilterString() {
        return mResultAdapter != null ? mResultAdapter.getLastFilterString()
                : null;
    }
    ...
    public void afterTextChanged(Editable s) {
        if (mClearButton != null) {
            mClearButton.setVisibility(s.length() > 0 ? View.VISIBLE
                    : View.GONE);
        }
    }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position,
        ...
        manager.hideSoftInputFromWindow(mAutoCompleteTextView.getWindowToken(),
                0);
        // An onClickListener for the view item because I haven't figured out a
        // way to update the AutoCompleteTextView without causing an infinite
        // loop.
    ...
    public void onClick(View v) {
        if (mAutoCompleteTextView != null
                && !mAutoCompleteTextView.isPopupShowing()) {
            filterOnString(mAutoCompleteTextView.getText().toString());
        }
    }

    // This method will set the adapter if no adapter has been set. The adapter
    // is initialized
    // here to prevent the drop-down from appearing uninvited on orientation
    // change, as the
    // AutoCompleteTextView.setText() will trigger the drop-down if the adapter
    // has been set.
