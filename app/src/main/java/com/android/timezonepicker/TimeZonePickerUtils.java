...
public class TimeZonePickerUtils {
    ...
    /**
     * This needs to be an instantiated class so that it doesn't need to
     * continuously re-load the list of timezone IDs that need to be overridden.
     * 
     * @param context
     */
    public TimeZonePickerUtils(Context context) {
        // Instead of saving a reference to the context (because we might need
        // to look up the
        // labels every time getGmtDisplayName is called), we'll cache the lists
        // of override IDs
        // and labels now.
    ...
    /**
     * Given a timezone id (e.g. America/Los_Angeles), returns the corresponding
     * timezone display name (e.g. Pacific Time GMT-7).
     * 
     * @param context
     *            Context in case the override labels need to be re-cached.
     * @param id
     *            The timezone id
     * @param millis
     *            The time (daylight savings or not)
     * @param grayGmt
     *            Whether the "GMT+x" part of the returned string should be
     *            colored gray.
     * @return The display name of the timezone.
     */
    public CharSequence getGmtDisplayName(Context context, String id,
            long millis, boolean grayGmt) {
        ...
        if (!defaultLocale.equals(mDefaultLocale)) {
            // If the IDs and labels haven't been set yet, or if the locale has
            // been changed
    ...
    /**
     * Gets the display name for the specified Timezone ID. If the ID matches
     * the list of IDs that need to be have their default display names
     * overriden, use the pre-set display name from R.arrays.
     * 
     * @param id
     *            The timezone ID.
     * @param daylightTime
     *            True for daylight time, false for standard time
     * @return The display name of the timezone. This will just use the default
     *         display name, except that certain timezones have poor defaults,
     *         and should use the pre-set override labels from R.arrays.
     */
    private String getDisplayName(TimeZone tz, boolean daylightTime) {
        if (mOverrideIds == null || mOverrideLabels == null) {
            ...
            return tz.getDisplayName(daylightTime, TimeZone.LONG,
                    Locale.getDefault());
        }
        ...
        // If the ID doesn't need to have the display name overridden, or if the
        // labels were
