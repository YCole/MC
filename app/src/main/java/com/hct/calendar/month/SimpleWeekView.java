...
public class SimpleWeekView extends View {
    ...
    protected int DEFAULT_HEIGHT = 56;
    ...
    protected String[] mDayInfos;
    protected String[] mWorkDayWeekDay;
    protected boolean[] mIsNormalDay;
    protected CalendarConvertTools mConvertTools;
    ...
    protected boolean mShowWeekIndex = false;
    ...
    public SimpleWeekView(Context context) {
        ...
        // HCT_MODIFY lixiange MF3.0 add lunar Calendar
        mConvertTools = new CalendarConvertTools(context);
        // HCT_MODIFY lixiange MF3.0 add lunar Calendar
        ...
        DEFAULT_HEIGHT = (int) res.getDimension(R.dimen.month_row_height);
    ...
    public void setWeekParams(HashMap<String, Integer> params, String tz) {
        ...
        if (params.containsKey(VIEW_PARAMS_HEIGHT)) {
            ...
            // mHeight = mWidth/7;
            mHeight = MonthFragment.itemHeight;
        ...
        if (params.containsKey(VIEW_PARAMS_SHOW_WK_NUM)) {
            if (params.get(VIEW_PARAMS_SHOW_WK_NUM) != 0) {
                ...
                mShowWeekIndex = false;
            }
        }
        mNumCells = mShowWeekIndex ? mNumDays + 1 : mNumDays;

        // Allocate space for caching the day numbers and focus values
        mDayNumbers = new String[mNumCells];

        // HCT_MODIFY lixiange MF3.0 add lunar Calendar
        mDayInfos = new String[mNumCells];
        mIsNormalDay = new boolean[mNumCells];
        mWorkDayWeekDay = new String[mNumCells];
        // HCT_MODIFY lixiange MF3.0 add lunar Calendar
        ...
        if (julianMonday <= Time.MONDAY_BEFORE_JULIAN_EPOCH)
            julianMonday++;
        ...
        int i = 0;
        if (mShowWeekIndex) {
            mDayNumbers[0] = Integer.toString(Utils.getWeekNumberFromCalendar(
                    time.toMillis(true), mContext));// time.getWeekNumber());
            i++;
        }
        ...
        for (; i < mNumCells; i++) {
            ...
            // HCT_MODIFY lixiange MF3.0 add lunar Calendar
            Time solarDate = new Time(time);
            Time lunarDate = mConvertTools.getLunarDateFromSolar(solarDate);
            String chnFesStr = mConvertTools
                    .getChineseFestivalFromSolar(solarDate);
            String chnHolStr = mConvertTools.getChnHolFromSolor(solarDate);
            String termString = mConvertTools
            ...
            if (lunarDate.monthDay == 1) {
                lunarDayStr = mConvertTools.getLunarMonthString(lunarDate);
            } else {
                lunarDayStr = mConvertTools.getLunarDayString(lunarDate);
            }

            String dayInfo = "";
            mIsNormalDay[i] = false;

            if (!chnFesStr.equals("")) {
                dayInfo = chnFesStr;
            } else if (!chnHolStr.equals("")) {
                String chnFesQingming = res.getString(R.string.chnFes_qingming);
                String ChnHolFuhuo = res.getString(R.string.ChnHol_fuhuo);
                if (chnHolStr.equals(ChnHolFuhuo)
                        && termString.equals(chnFesQingming)) {
                    dayInfo = termString;
                } else {
                    dayInfo = chnHolStr;
                }
            ...
            mDayInfos[i] = dayInfo;
            mWorkDayWeekDay[i] = workDayWeekDay;
            // HCT_MODIFY lixiange MF3.0 add lunar Calendar
    ...
    public Time getDayFromLocation(float x) {
        int dayStart = mShowWeekIndex ? (mWidth - mPadding * 2) / mNumCells
                + mPadding : mPadding;
