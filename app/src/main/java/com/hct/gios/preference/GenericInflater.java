 ...
 * @param P
 *            The type of parents (that is those items that contain other
 *            items). Must implement {@link GenericInflater.Parent}
 */
abstract class GenericInflater<T, P extends GenericInflater.Parent> {
    ...
    /**
     * Create a copy of the existing inflater object, with the copy pointing to
     * a different Context than the original. This is used by
     * {@link ContextThemeWrapper} to create a new inflater to go along with the
     * new Context theme.
     * 
     * @param newContext
     *            The new Context to associate with the new inflater. May be the
     *            same as the original Context if desired.
     * 
     * @return Returns a brand spanking new inflater object associated with the
     *         given Context.
     */
     ...
     *            The default package. This will be prepended to the tag name,
     *            so it should end with a period.
     */
     ...
     * inflater. This must not be null, and can only be set once; after setting,
     * you can not change the factory. This is called on each element name as
     * the XML is parsed. If the factory returns an item, that is added to the
     * hierarchy. If it returns null, the next factory default
     * {@link #onCreateItem} method is called.
     * <p>
     * If you have an existing inflater and want to add your own factory to it,
     * use {@link #cloneInContext} to clone the existing instance and then you
     ...
     * @return The root of the inflated hierarchy. If root was supplied, this is
     *         the root item; otherwise it is the root of the inflated XML file.
     */
     ...
     * reasons, inflation relies heavily on pre-processing of XML files that is
     * done at build time. Therefore, it is not currently possible to use
     * inflater with an XmlPullParser over a plain XML file at runtime.
     * 
     * @param parser
     ...
     *            Optional parent of the generated hierarchy.
     * @return The root of the inflated hierarchy. If root was supplied, this is
     *         the that; otherwise it is the root of the inflated XML file.
     */
     ...
     *            provides a set of values for root of the returned hierarchy
     *            (if <em>attachToRoot</em> is false.)
     * @param attachToRoot
    ...
    public T inflate(XmlPullParser parser, P root, boolean attachToRoot) {
        synchronized (mConstructorArgs) {
            ...
            try {
                ...
                T xmlRoot = createItemFromTag(parser,
                        changePreferenceName(parser.getName()), attrs);
    ...
    private String changePreferenceName(String name) {
        String PreferenceName = null;

        switch (name) {
        case "Preference":
            PreferenceName = "com.hct.gios.preference.PreferenceScreen";
            break;
        ...
        case "PreferenceCategory":
            PreferenceName = "com.hct.gios.preference.PreferenceCategory";
            break;

        case "EditTextPreference":
            PreferenceName = "com.hct.gios.preference.EditTextPreference";
            break;

        case "ListPreference":
            PreferenceName = "com.hct.gios.preference.ListPreference";
            break;

        case "MultiCheckPreference":
            PreferenceName = "com.hct.gios.preference.MultiCheckPreference";
            break;

        case "MultiSelectListPreference":
            PreferenceName = "com.hct.gios.preference.MultiSelectListPreference";
            break;

        case "SeekBarDialogPreference":
            PreferenceName = "com.hct.gios.preference.SeekBarDialogPreference";
            break;

        case "VolumePreference":
            PreferenceName = "com.hct.gios.preference.VolumePreference";
            break;

        case "CheckBoxPreference":
            PreferenceName = "com.hct.gios.preference.CheckBoxPreference";
            break;

        case "SwitchPreference":
            PreferenceName = "com.hct.gios.preference.SwitchPreference";
            break;

        case "RingtonePreference":
            PreferenceName = "com.hct.gios.preference.RingtonePreference";
            break;

        case "SeekBarPreference":
            PreferenceName = "com.hct.gios.preference.SeekBarPreference";
            break;
     ...
     * instantiate class of the given <var>name</var> found in this inflater's
     * ClassLoader.
     * 
     * <p>
    ...
    private void rInflate(XmlPullParser parser, T parent,
        ...
        int type;
        while (((type = parser.next()) != parser.END_TAG || parser.getDepth() > depth)
                && type != parser.END_DOCUMENT) {
            ...
            T item = createItemFromTag(parser,
                    changePreferenceName(parser.getName()), attrs);
