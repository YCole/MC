...
public class Preference implements Comparable<Preference> {
    ...
    public Preference(Context context, AttributeSet attrs, int defStyleAttr,
        ...
        for (int i = a.getIndexCount(); i >= 0; i--) {
            ...
            } else if (attr == R.styleable.Preference_android_isIndicator) {
                mIsIndicator = a.getBoolean(
                        R.styleable.Preference_android_isIndicator, false);
        ...
        if (!getClass().getName().startsWith("com.hct.gios.preference")
                && !getClass().getName().startsWith("com.hct.gios.widget")
                && !getClass().getName().startsWith("com.android")) {
            // For non-framework subclasses, assume the worst and don't cache
            // views.
    ...
    protected View onCreateView(ViewGroup parent) {
        ...
        if (layout.getParent() != null) {
            ViewGroup vgroup = (ViewGroup) layout.getParent();
            vgroup.removeView(layout);
        }
        mainLayout.addView(layout, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT,
                LinearLayout.LayoutParams.WRAP_CONTENT, 1));

        FrameLayout mframe = new FrameLayout(mContext);
        View divider = new View(mContext);
        divider.setId(R.id.divider);
        divider.setBackgroundResource(R.drawable.list_divider_material);
        MarginLayoutParams mp = null;
        mp = new FrameLayout.LayoutParams(
                FrameLayout.LayoutParams.MATCH_PARENT, 1);
        mp.leftMargin = Utils.dpToPx(mContext, 72);
        View photodivider = new View(mContext);
        photodivider.setId(R.id.photodivider);
        photodivider.setBackgroundResource(R.drawable.list_divider_material);
        mframe.addView(divider, new FrameLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, 1));
        mframe.addView(photodivider, mp);
        photodivider.setVisibility(View.GONE);
        if (mframe.getParent() != null) {
            ViewGroup vgroup = (ViewGroup) mframe.getParent();
            vgroup.removeView(mframe);
        }
        if (this instanceof PreferenceCategory) {
            divider.setVisibility(View.GONE);
        }
        mainLayout.addView(mframe, new LinearLayout.LayoutParams(
                LinearLayout.LayoutParams.MATCH_PARENT, 1));
    ...
    protected void onBindView(View view) {
        ...
        if (summaryView != null) {
            ...
            if (!TextUtils.isEmpty(summary)) {
                ...
                if (view.findViewById(R.id.text_container) != null)
                    view.findViewById(R.id.text_container).setPadding(0,
                            Utils.dpToPx(mContext, 16), 0,
                            Utils.dpToPx(mContext, 16));
        ...
        if (mImageView != null) {
            if (mIconResId != 0 || mIcon != null) {
                ...
                if (mIcon != null) {
                    ...
                    {
                        Log.d("preferecne", "IMAGE width=" + mWidth + "height="
                                + mHight);
                        mImageView
            ...
            if (mIcon == null) {
                view.findViewById(R.id.photodivider).setVisibility(View.GONE);
                view.findViewById(R.id.divider).setVisibility(View.VISIBLE);
            } else {
                if (isLastPreference == 0) {
                    view.findViewById(R.id.photodivider).setVisibility(
                            View.VISIBLE);
                    view.findViewById(R.id.divider).setVisibility(View.GONE);
                } else if (isLastPreference == 1) {
                    view.findViewById(R.id.photodivider).setVisibility(
                            View.GONE);
                    view.findViewById(R.id.divider).setVisibility(View.VISIBLE);
                }
            ...
            mImageView.setVisibility(mIcon != null ? View.VISIBLE : View.GONE);
        ...
        final View imageIndicator = view.findViewById(R.id.indicator);
        {

            if (imageIndicator != null) {
                if (mIsIndicator == true && mEnabled == false) {

                    Drawable dr = getContext().getDrawable(
                            R.drawable.prference_go_disable);
                    dr.setAutoMirrored(true);
                    ((ImageView) imageIndicator).setImageDrawable(dr);

                } else {
                    Drawable dr = getContext().getDrawable(
                            R.drawable.prference_go);
                    dr.setAutoMirrored(true);
                    ((ImageView) imageIndicator).setImageDrawable(dr);
                }
                imageIndicator
                        .setVisibility(mIsIndicator == true ? View.VISIBLE
                                : View.GONE);
            }
        }
    ...
    public void setIcon(Drawable icon, int width, int height) {
        if ((icon == null && mIcon != null) || (icon != null && mIcon != icon)) {
            ...
            Log.d("preferecne", "SetICon width=" + width + "height=" + height);
    ...
    public void setIcon(int iconResId, int width, int height) {
        mIconResId = iconResId;

        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.L) {
            setIcon(mContext.getDrawable(iconResId), width, height);
        } else {
            setIcon(mContext.getResources().getDrawable(iconResId), width,
                    height);
        }
    }
