...
import com.hct.gios.preference.Preference.OnPreferenceChangeInternalListener;
...
/**
 * An adapter that returns the {@link Preference} contained in this group. In
 * most cases, this adapter should be the base class for any custom adapters
 * from {@link Preference#getAdapter()}.
 * <p>
 * This adapter obeys the {@link Preference}'s adapter rule (the
 * {@link Adapter#getView(int, View, ViewGroup)} should be used instead of
 * {@link Preference#getView(ViewGroup)} if a {@link Preference} has an adapter
 * via {@link Preference#getAdapter()}).
 * <p>
 ...
 * @hide
 */
public class PreferenceGroupAdapter extends BaseAdapter implements
    ...
    private void flattenPreferenceGroup(List<Preference> preferences,
        ...
        for (int i = 0; i < groupSize; i++) {
            ...
            if (i < (groupSize - 1)) {
                Preference preferenceNext = group.getPreference(i + 1);
                if (preferenceNext instanceof PreferenceCategory) {
                    preference.isLastPreference = 1;
                }
            } else {
                preference.isLastPreference = 1;
            }
    ...
    /**
     * Creates a string that includes the preference name, layout id and widget
     * layout id. If a particular preference type uses 2 different resources,
     * they will be treated as different view types.
     */
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        // Build a PreferenceLayout to compare with known ones that are
        // cacheable.
        mTempPreferenceLayout = createPreferenceLayout(preference,
                mTempPreferenceLayout);

        // If it's not one of the cached ones, set the convertView to null so
        // that
    ...
    public boolean areAllItemsEnabled() {
        // There should always be a preference group, and these groups are
        // always
        // disabled
        return false;
    }
