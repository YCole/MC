...
abstract class DrawerArrowDrawable extends Drawable {
    ...
    DrawerArrowDrawable(Context context) {
        ...
        mSize = context.getResources().getDimensionPixelSize(
                R.dimen.drawableSize);// typedArray.getDimensionPixelSize(R.styleable.DrawerArrowToggle_drawableSize,
                                      // 0);
        mBarSize = context.getResources().getDimension(R.dimen.barSize);// typedArray.getDimension(R.styleable.DrawerArrowToggle_barSize,
                                                                        // 0);
        mTopBottomArrowSize = context.getResources().getDimension(
                R.dimen.topBottomBarArrowSize);// typedArray
        // .getDimension(R.styleable.DrawerArrowToggle_topBottomBarArrowSize,
        // 0);
        mBarThickness = context.getResources().getDimension(R.dimen.thickness);// typedArray.getDimension(R.styleable.DrawerArrowToggle_thickness,
                                                                               // 0);
        mBarGap = context.getResources().getDimension(R.dimen.gapBetweenBars);// typedArray.getDimension(R.styleable.DrawerArrowToggle_gapBetweenBars,
                                                                              // 0);
        mSpin = true;// typedArray.getBoolean(R.styleable.DrawerArrowToggle_spinBars,
                     // true);
        mMiddleArrowSize = context.getResources().getDimension(
                R.dimen.middleBarArrowSize);// typedArray
    ...
    public void draw(Canvas canvas) {
        ...
        final float canvasRotate = lerp(isRtl ? 0 : -180, isRtl ? 180 : 0,
                mProgress);
        final float topBottomBarOffset = lerp(mBarGap + mBarThickness, 0,
                mProgress);
        mPath.rewind();
