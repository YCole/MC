...
public class OverlayDrawer extends DraggableDrawer {
    ...
    public void SetDrawerDisable(boolean isDisable) {
        mDisable = isDisable;
    }
    ...
    public void toggleMenu(Intent intent) {
        startIntent = intent;
        toggleMenu(true);
    }
    ...
    protected void onOffsetPixelsChanged(int offsetPixels) {
        ...
        if (USE_TRANSLATIONS) {
            ...
            case BOTTOM:
                mMenuContainer.offsetTopAndBottom(offsetPixels
                        - (mMenuContainer.getTop() - getHeight()));
                break;

            case LEFT:
                mMenuContainer.offsetLeftAndRight(offsetPixels
                        - mMenuContainer.getRight());
                break;

            case RIGHT:
                mMenuContainer.offsetLeftAndRight(offsetPixels
                        - (mMenuContainer.getLeft() - getWidth()));
                break;
            }
        }
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (USE_TRANSLATIONS) {
            ...
            switch (mPosition) {
            case LEFT:
                mMenuContainer.layout(-menuSize + offsetPixels, 0,
                        offsetPixels, height);
                break;

            case RIGHT:
                mMenuContainer.layout(width + offsetPixels, 0, width + menuSize
                        + offsetPixels, height);
                break;

            case TOP:
                mMenuContainer.layout(0, -menuSize + offsetPixels, width,
                        offsetPixels);
                break;

            case BOTTOM:
                mMenuContainer.layout(0, height + offsetPixels, width, height
                        + menuSize + offsetPixels);
                break;
            }
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
         ...
         * IllegalStateException("Must measure with an exact size"); }
         */
    ...
    protected boolean onMoveAllowDrag(int x, int y, float dx, float dy) {
        ...
        switch (mPosition) {
        case LEFT:
            return (!mMenuVisible && mInitialMotionX <= mTouchSize && (dx > 0)) // Drawer
                                                                                // closed
                    || (mMenuVisible && x <= mOffsetPixels) // Drawer open
                    || (Math.abs(mOffsetPixels) <= mPeekSize && mMenuVisible); // Drawer
                                                                               // revealed
