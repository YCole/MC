...
class DayPickerView extends ListView implements AbsListView.OnScrollListener,
    ...
    private static int LIST_TOP_OFFSET = -1; // so that the top line will be
                                             // under the separator
    ...
    private int getDiffMonths(Calendar start, Calendar end) {
        ...
        final int diffMonths = end.get(Calendar.MONTH)
                - start.get(Calendar.MONTH) + 12 * diffYears;
        return diffMonths;
    }

    private int getPositionFromDay(Calendar day) {
        final int diffMonthMax = getDiffMonths(mController.getMinDate(),
                mController.getMaxDate());
    ...
    private Calendar findAccessibilityFocus() {
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            if (child instanceof SimpleMonthView) {
                final Calendar focus = ((SimpleMonthView) child)
    ...
    private String getMonthAndYearString(Calendar day) {
        ...
        sbuf.append(day.getDisplayName(Calendar.MONTH, Calendar.LONG,
                Locale.getDefault()));
        sbuf.append(" ");
